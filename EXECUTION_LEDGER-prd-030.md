...
> dreaming); optionally also a dreaming-pass step when PRD-026 is enabled.
...
- **Poll-convergent resolve** posture: `dreaming/trigger.ts` `RESOLVE_POLLS` + `readState` (keep MAX across a bounded poll union; append-only versions are monotone so a single read can only under-report). MANDATORY for resolving highest-version-per-key and every read-back assertion (DeepLake flaps stale segments — project memory note).
- **SQL guards**: every value via `sLiteral`/`val.*`, every identifier via `sqlIdent` (`storage/sql.ts`). `audit:sql` scans `src/daemon`.
- **Throwaway-table isolation**: the live counter smoke + the dreaming-counter itest point the trigger at a per-run namespaced `ci_*_<runId>` table via a `tableName` seam and DROP it in `afterAll`. The compaction live itest reuses this — NEVER a real `dreaming_state`/skills/rules table.
- **Scope (D-6)**: version-bumped tables ONLY (skills, rules, claim history, `dreaming_state`-style counters). NOT `appendOnlyInsert` event tables (sessions/raw events — no version concept; retention is PRD-007's concern).
...
- D-2 PRIMARY = standalone maintenance job via a `honeycomb maintenance` verb (runs regardless of premium dreaming); OPTIONAL = also a step of a dreaming pass when 026 enabled.
...
- NEW `src/daemon/storage/compaction.ts` (storage-level — NOT `runtime/dreaming/compaction.ts`, which is graph-prompt assembly): a guarded version-history reaper.
  ...
  - Retention config (zod, conservative defaults N=5 + 30d) mirroring `dreaming/config.ts` coerce-and-clamp.
...
- A daemon is running on 127.0.0.1:3850 with dreaming enabled — leave it running.
