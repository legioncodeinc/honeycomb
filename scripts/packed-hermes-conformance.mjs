...
try {
	...
	for (const file of [
		"honeycomb/bundle/session-start.js",
		"honeycomb/bundle/capture.js",
		"honeycomb/bundle/session-end.js",
		"honeycomb/mcp/server.js",
	]) {
	...
	for (const hookCase of hookCases) {
		const result = spawnSync(
			process.execPath,
			[join(hermesHome, "honeycomb", "bundle", hookCase.file), ...hookCase.args],
			{ encoding: "utf8", env, input: JSON.stringify(hookCase.payload) },
		);
	...
	const conflict = spawnSync(process.execPath, [cli, "connect", "hermes", "--no-color"], {
		encoding: "utf8",
