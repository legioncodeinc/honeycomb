 ...
 * plugin bundle (PRD-001b FR-9 / b-AC-4). Runs the same per-file regex rules
 * ClawHub uses at publish time so flags surface BEFORE shipping a release.
 *
 * The two rules that matter for Honeycomb's OpenClaw bundle:
 *   - `dangerous-exec` (critical): a child_process exec call — eliminated by
 *     the esbuild stub-unused-child-process plugin.
 *   - `env-harvesting` (critical): a `process.env` read near a network send —
 *     eliminated by the esbuild define that rewrites process.env.HONEYCOMB_*
 *     to globalThis.__honeycomb_tuning__.HONEYCOMB_*.
 *
 * Usage:
 ...
 *   node scripts/audit-openclaw-bundle.mjs --criticals-only
 *
 * Exits non-zero if any finding (strict, default) or any critical finding
 * (--criticals-only) is reported.
 */
    ...
    return [
      {
        ...
        file: path,
        line: 0,
        message: `Skipped (${stat.size} bytes > ${MAX_FILE_BYTES} byte limit)`,
        evidence: "",
      },
    ];
  }
...
const SEVERITY_ICON = { info: "·", warn: "!", critical: "x" };
...
if (allFindings.length === 0) {
  console.log("OK - no findings. Bundle is clean against ClawHub's static-analysis rules.\n");
  process.exit(0);
}
...
if (!STRICT_MODE && counts.critical === 0 && counts.warn > 0) {
  console.log(
    "(--criticals-only: warns are advisory and do NOT block. Re-run without the flag for strict local checks.)\n",
  );
}
const shouldFail = STRICT_MODE ? counts.critical > 0 || counts.warn > 0 : counts.critical > 0;
