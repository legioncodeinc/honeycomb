...
const INSTALLED_MCP = `${HOME}/.hermes/honeycomb/mcp/server.js`;
...
function seedFs(config?: string) {
	return createFakeFs({
		files: {
			...
			[`${BUNDLE}/session-end.js`]: "// session-end",
			[MCP]: "// mcp server",
...
describe("HermesConnector", () => {
	it("extends the shared connector base and wires Hermes' native YAML hook + MCP contract", async () => {
		...
		expect(result.handlers).toContain(`${PLUGIN}/session-start.js`);
		expect(result.handlers).toContain(`${PLUGIN}/capture.js`);
		expect(result.handlers).toContain(`${PLUGIN}/session-end.js`);
		...
		expect(config.hooks?.pre_llm_call?.some((h) => h.command.includes("--honeycomb-recall"))).toBe(true);
	...
	it("refuses a foreign honeycomb-named MCP server before writing owned artifacts", async () => {
		...
		expect(fs.files.has(`${PLUGIN}/capture.js`)).toBe(false);
	...
	it("honors an explicit profile-aware Hermes home", async () => {
		...
		expect(fs.files.has(`${hermesHome}/honeycomb/bundle/capture.js`)).toBe(true);
	});
