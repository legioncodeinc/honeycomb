/**
 * PRD-026 Wave 1 Track B — the daemon-resident POLLINATING JOB WORKER (AC-W).
 *
 * These tests pin the WORKER contract: it leases ONLY `["pollinating"]` (never a foreign
 ...
 *   - A FAKE queue holding the pollinating job, recording the `kinds` arg `lease` got and
 ...
 *     we can assert the `memory_pollinating` workload WAS called.
...
} from "../../../../src/daemon/runtime/pollinating/worker.js";
import type { PollinatingScope, PollinatingState } from "../../../../src/daemon/runtime/pollinating/trigger.js";
...
/** A resolved pollinating config with overridable knobs (the worker only reads three). */
function config(over: Partial<PollinatingConfig> = {}): PollinatingConfig {
	return {
		enabled: true,
...
class FakeTrigger implements PollinatingTriggerSeam {
	readonly recorded: { agentId: string; passAt: string }[] = [];
	readonly readScopes: PollinatingScope[] = [];
...
/** A pollinating job as it sits on the queue (payload is the trigger's enqueue body). */
function pollinatingJob(mode: "incremental" | "compaction", agentId = "agent-alpha"): LeasedJob {
	return {
		id: "job-1",
		kind: "pollinating",
		attempt: 1,
...
describe("AC-W pollinating worker — runOnce leases ['pollinating'], runs the runner, completes", () => {
	it("a single pollinating job → memory_pollinating called, runner ran, complete() once", async () => {
		const { storage, transport } = storageWith(oneSummary);
		const queue = new FakeQueue(pollinatingJob("incremental"));
		const model = createFakeModelClient({ memory_pollinating: CANNED_BODY });
		const trigger = new FakeTrigger("2025-12-01T00:00:00.000Z"); // has a prior pass → incremental.

		const worker = createPollinatingWorker({ queue, storage, scope: SCOPE, config: config(), model, trigger });
		const processed = await worker.runOnce();

		expect(processed).toBe(true);
		// The model WAS called with the memory_pollinating workload (the stronger target).
	...
	it("leases with the ['pollinating'] kind filter — never a generic lease", async () => {
		...
		// The worker passed exactly ["pollinating"] to lease — the foreign-kind guard.
		expect(queue.leaseKinds).toHaveLength(1);
		expect(queue.leaseKinds[0]).toEqual(["pollinating"]);
	});

	it("a foreign-kind job in the queue is never leased by the pollinating worker", async () => {
		const { storage } = storageWith(allNew);
...
describe("AC-W pollinating worker — malformed payload fails the job (never silent complete)", () => {
	it("an unparseable pollinating payload → queue.fail, never complete, never the model", async () => {
		const { storage } = storageWith(allNew);
		// `mode` is not a valid enum member → parsePollinatingJobPayload returns null.
...
describe("AC-W pollinating worker — mode selection (D-4 / backfill-first-run)", () => {
	it("first run (backfillOnFirstRun + empty last_pass_at) selects COMPACTION", async () => {
		...
		const model = createFakeModelClient({ memory_pollinating: '{"mutations":[],"summary":"compacted"}' });
		...
		expect(model.calls[0].prompt).toContain("compaction pollinating agent");
		expect(queue.completed).toEqual(["job-1"]);
	...
	it("steady state (prior last_pass_at) selects INCREMENTAL", async () => {
		...
		const queue = new FakeQueue(pollinatingJob("incremental"));
		const model = createFakeModelClient({ memory_pollinating: '{"mutations":[],"summary":"incr"}' });
		...
		expect(model.calls).toHaveLength(1);
		// The incremental prompt carries the POLLINATING.md task framing, NOT the compaction preamble.
		expect(model.calls[0].prompt).not.toContain("compaction pollinating agent");
		expect(model.calls[0].prompt).toContain("POLLINATING");
		expect(queue.completed).toEqual(["job-1"]);
	});
	...
	it("an explicit compaction-mode payload selects COMPACTION regardless of last_pass_at", async () => {
		const withGraph = (req: TransportRequest): StorageRow[] =>
			/FROM\s+"?entities"?/i.test(req.sql) ? [{ id: "ent_1", name: "Acme", type: "company" }] : [];
		const sg = storageWith(withGraph);
		const queue = new FakeQueue(pollinatingJob("compaction"));
		const model = createFakeModelClient({ memory_pollinating: '{"mutations":[],"summary":"compacted"}' });
...
describe("AC-W pollinating worker — factory constructs without starting (assembly owns the gate)", () => {
	it("createPollinatingWorker builds a worker that has NOT polled the queue", async () => {
		const { storage } = storageWith(allNew);
		const queue = new FakeQueue(pollinatingJob("incremental"));
		const worker = createPollinatingWorker({
			queue,
