/**
 * PRD-009a Pollinating Token-Budget Trigger — a-AC-1..6 (Wave 1).
 *
 * Verification posture (EXECUTION_LEDGER-prd-009 / pollinating CONVENTIONS):
 *   - All assertions run against a STATEFUL fake DeepLake transport that models the
 *     `pollinating_state` table's append-only version-bump semantics in memory (INSERT
...
import { PollinatingConfigSchema, type PollinatingConfig } from "../../../../src/daemon/runtime/pollinating/config.js";
import {
...
import { POLLINATING_JOB_KIND, parsePollinatingJobPayload } from "../../../../src/daemon/runtime/pollinating/contracts.js";

const SCOPE: QueryScope = { org: "test-org", workspace: "test-ws" };

function config(overrides: Partial<PollinatingConfig> = {}): PollinatingConfig {
	return PollinatingConfigSchema.parse({ enabled: true, tokenThreshold: 100, ...overrides });
}

// ── A stateful in-memory pollinating_state store + responder ─────────────────────
...
/**
 * Models the `pollinating_state` append-only table in memory. An INSERT appends a row;
...
class FakeQueue implements PollinatingJobEnqueuer {
...
function storageOf(store: PollinatingStore): StorageQuery {
...
const AGENT: PollinatingScope = { agentId: "agent-alpha" };
...
describe("a-AC-1 session-summary write increments tokens_since_last_pass by the token count", () => {
	it("two increments accumulate the running token count (append-only)", async () => {
		const store = new PollinatingStore();
		const trigger = createPollinatingTrigger({
		...
		expect(await trigger.incrementPollinatingCounter(AGENT, 30)).toBe(30);
		expect(await trigger.incrementPollinatingCounter(AGENT, 12)).toBe(42);
	...
	it("a negative/garbage token count floors to 0 (a summary never decrements)", async () => {
		...
		await trigger.incrementPollinatingCounter(AGENT, 50);
		expect(await trigger.incrementPollinatingCounter(AGENT, -999)).toBe(50);
		expect(await trigger.incrementPollinatingCounter(AGENT, Number.NaN)).toBe(50);
	});
});
...
describe("a-AC-2 threshold crossed → exactly one pollinating job enqueued + counter resets (SUBTRACT)", () => {
	it("enqueues one job, records pending_job_id, and SUBTRACTS the threshold (not hard-zero)", async () => {
		...
		await trigger.incrementPollinatingCounter(AGENT, 130);

		const result = await trigger.checkAndEnqueuePollinating(AGENT);
		expect(result.decision).toBe("enqueued");
		expect(result.jobId).toBe("job-1");

		// Exactly one enqueue, of the pollinating kind, carrying the mode + agent scope.
		expect(queue.calls).toHaveLength(1);
		expect(queue.calls[0].kind).toBe(POLLINATING_JOB_KIND);
		const payload = parsePollinatingJobPayload(queue.calls[0].payload);
	...
	it("a summary write between the threshold read and the reset is not lost (SUBTRACT carries overflow)", async () => {
		...
		await trigger.incrementPollinatingCounter(AGENT, 250);
		await trigger.checkAndEnqueuePollinating(AGENT);
...
describe("a-AC-3 a pending pass blocks a second enqueue until the prior job is terminal", () => {
	it("a second tick at threshold enqueues NOTHING while a job is pending", async () => {
		...
		await trigger.incrementPollinatingCounter(AGENT, 130);
		await trigger.checkAndEnqueuePollinating(AGENT); // enqueues job-1, pending set.

		// More summaries push back over threshold, but a pass is pending.
		await trigger.incrementPollinatingCounter(AGENT, 200);
		const second = await trigger.checkAndEnqueuePollinating(AGENT);
	...
	it("once the pending job is terminal, the guard clears and a later tick can enqueue again", async () => {
		...
		await trigger.incrementPollinatingCounter(AGENT, 130);
		await trigger.checkAndEnqueuePollinating(AGENT); // job-1.

		// The prior job reaches a terminal state; first tick CLEARS the guard.
		terminal = true;
		const cleared = await trigger.checkAndEnqueuePollinating(AGENT);
		...
		await trigger.incrementPollinatingCounter(AGENT, 100);
		const again = await trigger.checkAndEnqueuePollinating(AGENT);
...
describe("a-AC-4 pollinating.enabled false → counter still grows but no job is enqueued", () => {
	it("increments past the threshold but a tick enqueues nothing", async () => {
		...
		await trigger.incrementPollinatingCounter(AGENT, 500); // well over threshold.
		const result = await trigger.checkAndEnqueuePollinating(AGENT);
...
describe("a-AC-5 a daemon restart reads back the counter from committed writes (durable)", () => {
	it("a fresh trigger over the SAME store reads the accumulated counter", async () => {
		const store = new PollinatingStore();
		const t1 = createPollinatingTrigger({
			storage: storageOf(store),
			scope: SCOPE,
			config: config(),
			enqueuer: new FakeQueue(),
		});
		await t1.incrementPollinatingCounter(AGENT, 40);
		await t1.incrementPollinatingCounter(AGENT, 35); // committed = 75 across two versions.

		// "Restart": a brand-new trigger + storage client over the SAME committed rows.
		const t2 = createPollinatingTrigger({
			storage: storageOf(store),
			scope: SCOPE,
			config: config(),
			enqueuer: new FakeQueue(),
...
describe("a-AC-6 two agent_ids under one workspace accumulate independent counters", () => {
	it("each agent's counter is isolated by the agent_id ring + a distinct row id", async () => {
		...
		const alpha: PollinatingScope = { agentId: "agent-alpha" };
		const beta: PollinatingScope = { agentId: "agent-beta" };

		await trigger.incrementPollinatingCounter(alpha, 100);
		await trigger.incrementPollinatingCounter(beta, 7);
		...
		expect(pollinatingStateId(alpha)).not.toBe(pollinatingStateId(beta));
	});
});
