/**
 * PRD-009b Pollinating Session Runner (incremental pass) — b-AC-1..6 (Wave 2).
 *
 * Verification posture (EXECUTION_LEDGER-prd-009 / pollinating CONVENTIONS):
 *   - The full {@link PollinatingRunner} lifecycle runs against a STATEFUL fake DeepLake
 *     transport that models the relevant tables (`pollinating_state`, `memory`,
 ...
 *   - The model is a FAKE `ModelClient` (`createFakeModelClient({ memory_pollinating })`)
 *     so the pollinating workload + the returned mutation set are scripted and asserted.
 ...
 *
 * b-AC-1 payload loads identity + new summaries + graph snapshot + POLLINATING.md; the
 ...
 * b-AC-6 the model call resolves the `memory_pollinating` workload, not extraction.
 */
...
} from "../../../../src/daemon/runtime/pollinating/runner.js";
import type { PollinatingJobPayload } from "../../../../src/daemon/runtime/pollinating/contracts.js";
import {
	createGraphQueryTool,
	createIncrementalStrategy,
	DEFAULT_POLLINATING_TASK_PROMPT,
	estimateTokens,
	IncrementalPayloadStrategy,
	type PollinatingIdentitySource,
} from "../../../../src/daemon/runtime/pollinating/incremental.js";
import { pollinatingStateId } from "../../../../src/daemon/runtime/pollinating/trigger.js";
...
function recordingUpdater(): PollinatingStateUpdater & { calls: Array<{ agentId: string; passAt: string }> } {
...
/** A fixed identity source so the identity/POLLINATING.md seam is asserted deterministically. */
function identitySource(over: Partial<{
	identityFiles: string[];
	priorPollinatingSessions: string[];
	memoryMd: string;
	pollinatingTaskPrompt: string;
}> = {}): PollinatingIdentitySource {
	return {
		load() {
			return Promise.resolve({
				identityFiles: over.identityFiles ?? ["IDENTITY: I am the pollinating agent."],
				priorPollinatingSessions: over.priorPollinatingSessions ?? ["PRIOR PASS: merged A and B."],
				memoryMd: over.memoryMd ?? "MEMORY.md body",
				pollinatingTaskPrompt: over.pollinatingTaskPrompt ?? DEFAULT_POLLINATING_TASK_PROMPT,
			});
		},
	};
}

const JOB: PollinatingJobPayload = {
	mode: "incremental",
	agentId: AGENT,
...
/** Seed a prior `pollinating_state` row carrying a `last_pass_at` boundary for the scope. */
function seedLastPass(store: GraphStore, lastPassAt: string): void {
	store.seed("pollinating_state", [
		{
			id: pollinatingStateId({ agentId: AGENT }),
			agent_id: AGENT,
...
describe("b-AC-1: pass loads identity + new summaries + graph snapshot + POLLINATING.md", () => {
	it("assembles a prompt carrying the identity preset, POLLINATING.md, new summaries, and the changed graph", async () => {
		...
		expect(prompt).toContain("POLLINATING.md (task)");
		expect(prompt).toContain(DEFAULT_POLLINATING_TASK_PROMPT.slice(0, 32));
		expect(prompt).toContain("IDENTITY: I am the pollinating agent.");
	...
	it("captures the pass as a session: the harness drives load → model → apply → finalize for a real job", async () => {
		...
		const model = createFakeModelClient({
			memory_pollinating: JSON.stringify({
				mutations: [
...
describe("b-AC-2: mutations apply via the control plane with provenance; destructive → pending review", () => {
	async function runWith(store: GraphStore, mutations: unknown[]): ReturnType<ReturnType<typeof createPollinatingRunner>["runPass"]> {
		seedLastPass(store, PASS_BOUNDARY);
		seedSummary(store, "sum/new", "trigger summary", "2026-06-15T00:00:00.000Z");
		const model = createFakeModelClient({
			memory_pollinating: JSON.stringify({ mutations, summary: "s", tokenBudget: 128_000 }),
		});
		const runner = createPollinatingRunner({
			storage: storageFor(store),
			scope: SCOPE,
...
describe("b-AC-5: on a successful pass, last_pass_at is updated and pending_job_id is cleared", () => {
	it("invokes the state updater exactly once with the agent id and a fresh ISO last_pass_at", async () => {
		...
		const model = createFakeModelClient({
			memory_pollinating: JSON.stringify({ mutations: [], summary: "noop", tokenBudget: 128_000 }),
		});
		const updater = recordingUpdater();
		const runner = createPollinatingRunner({
			storage: storageFor(store),
	...
	it("still finalizes (stamps + clears) on an empty pass when there is nothing to pollinate over", async () => {
		const store = new GraphStore();
		seedLastPass(store, PASS_BOUNDARY);
		// No new summaries → strategy returns null → harness records an empty pass.
		const model = createFakeModelClient({ memory_pollinating: "{}" });
		const updater = recordingUpdater();
		const runner = createPollinatingRunner({
			storage: storageFor(store),
			scope: SCOPE,
...
describe("b-AC-6: the model call resolves the pollinating workload, not extraction", () => {
	it("calls the model with the memory_pollinating workload", async () => {
		const store = new GraphStore();
		seedLastPass(store, PASS_BOUNDARY);
		seedSummary(store, "sum/new", "fact for routing", "2026-06-15T00:00:00.000Z");

		const model = createFakeModelClient({
			memory_pollinating: JSON.stringify({ mutations: [], summary: "", tokenBudget: 128_000 }),
		});
		const runner = createPollinatingRunner({
			storage: storageFor(store),
			scope: SCOPE,
		...
		expect(model.calls[0].workload).toBe("memory_pollinating");
...
describe("token budgeting: the payload is capped to maxInputTokens (D-2)", () => {
	...
	it("caps the assembled prompt under a tiny maxInputTokens budget", async () => {
		...
		// POLLINATING.md leads the priority order, so the task framing survives the cap.
		expect(payload?.prompt ?? "").toContain("POLLINATING.md (task)");
	});
});
