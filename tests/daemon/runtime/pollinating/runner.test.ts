/**
 * PRD-009 Pollinating Session-Runner HARNESS — the Wave-1 contract Wave 2 builds on.
 *
 ...
 *   - a non-null payload drives the `memory_pollinating` workload (the stronger target);
...
function fakeStrategy(payload: PollinatingPayload | null, mode: PollinatingPassMode = "incremental"): PollinatingPayloadStrategy {
...
const JOB: PollinatingJobPayload = { mode: "incremental", agentId: "agent-alpha", enqueuedAt: "", tokensAtEnqueue: 0 };

describe("pollinating runner harness — payload-strategy seam", () => {
	it("a null payload completes an EMPTY pass: no model call, state still updated", async () => {
		...
		const model = createFakeModelClient({ memory_pollinating: '{"mutations":[],"summary":"x"}' });
...
describe("pollinating runner harness — model workload + 008c apply seam (D-6 / b-AC-2 / b-AC-6)", () => {
	it("calls the memory_pollinating workload and routes destructive ops to PENDING review", async () => {
		const { storage, transport } = storageWith(allNew);
		...
		const model = createFakeModelClient({ memory_pollinating: body });
		const updater = new FakeStateUpdater();
		const runner = createPollinatingRunner({
			storage,
			scope: SCOPE,
			strategy: fakeStrategy({ prompt: "pollinate...", tokenBudget: 128_000 }),
			model,
			stateUpdater: updater,
		});
		...
		// b-AC-6: the stronger pollinating workload was used.
	...
	it("a malformed / truncated model body never fails the pass (drop-invalid)", async () => {
		...
		const model = createFakeModelClient({ memory_pollinating: '<think>reasoning</think>{ "mutations": [ {"kind":' }); // truncated JSON.
		const updater = new FakeStateUpdater();
		const runner = createPollinatingRunner({
			storage,
			scope: SCOPE,
			strategy: fakeStrategy({ prompt: "pollinate...", tokenBudget: 1000 }),
			model,
			stateUpdater: updater,
		});
	...
	it("strips a CoT block + fence and applies the embedded mutation set", async () => {
		...
		const model = createFakeModelClient({ memory_pollinating: wrapped });
		const runner = createPollinatingRunner({
			storage,
			scope: SCOPE,
