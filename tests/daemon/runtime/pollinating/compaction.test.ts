 ...
 *   - A FAKE PollinateJobEnqueuer records calls (c-AC-2 CLI test).
 *   - A FAKE PollinatingStateUpdater records calls (c-AC-4 post-compaction state).
 ...
 * c-AC-2 `pollinate trigger --compact` → job enqueued regardless of counter state.
...
import {
	...
	resolvePassMode,
} from "../../../../src/daemon/runtime/pollinating/compaction.js";
import { PollinatingConfigSchema, type PollinatingConfig } from "../../../../src/daemon/runtime/pollinating/config.js";
import {
	parsePollinateArgs,
	runPollinateCommand,
	type PollinateJobEnqueuer,
	type PollinateScope,
	type PollinateStateReader,
	type PollinateStateSnapshot,
} from "../../../../src/cli/pollinate.js";
...
function config(overrides: Partial<PollinatingConfig> = {}): PollinatingConfig {
	return PollinatingConfigSchema.parse({
		enabled: true,
...
function compactionJob(overrides: Partial<PollinatingJobPayload> = {}): PollinatingJobPayload {
...
class FakeStateUpdater implements PollinatingStateUpdater {
...
class FakeEnqueuer implements PollinateJobEnqueuer {
	readonly compactionCalls: PollinateScope[] = [];
	async enqueueCompaction(scope: PollinateScope): Promise<string> {
...
function fakeReader(snapshot: PollinateStateSnapshot | null): PollinateStateReader {
...
// ── c-AC-2: `pollinate trigger --compact` enqueues regardless of counter ─────────

describe("c-AC-2: pollinate trigger --compact — enqueues regardless of token counter", () => {
	it("parsePollinateArgs: trigger + --compact sets subCommand='trigger' compact=true", () => {
		const inv = parsePollinateArgs(["trigger", "--compact", "--org", "acme", "--workspace", "main"]);
	...
	it("parsePollinateArgs: defaults agentId to 'default' when --agent not supplied", () => {
		const inv = parsePollinateArgs(["trigger", "--compact"]);
		expect(inv.scope.agentId).toBe("default");
	});

	it("runPollinateCommand trigger --compact: enqueues a compaction job and returns job id", async () => {
		const enqueuer = new FakeEnqueuer();
		const reader = fakeReader(null);
		const { lines, sink } = captureOutput();

		const inv = parsePollinateArgs(["trigger", "--compact", "--org", "acme", "--workspace", "ws1", "--agent", "agent-x"]);
		const result = await runPollinateCommand(inv, enqueuer, reader, sink);
	...
	it("runPollinateCommand trigger --compact: enqueues regardless of any counter value (counter not consulted)", async () => {
	...
	it("runPollinateCommand trigger without --compact: refuses with exit 2", async () => {
		const enqueuer = new FakeEnqueuer();
		const reader = fakeReader(null);
		const { lines, sink } = captureOutput();

		const inv = parsePollinateArgs(["trigger"]);
		const result = await runPollinateCommand(inv, enqueuer, reader, sink);
	...
	it("runPollinateCommand status: shows state snapshot", async () => {
		const enqueuer = new FakeEnqueuer();
		const snapshot: PollinateStateSnapshot = {
		...
		const inv = parsePollinateArgs(["status", "--org", "acme"]);
		const result = await runPollinateCommand(inv, enqueuer, reader, sink);
	...
	it("runPollinateCommand status with no prior state: reports no pass on record", async () => {
		const enqueuer = new FakeEnqueuer();
		const reader = fakeReader(null);
		const { lines, sink } = captureOutput();

		const inv = parsePollinateArgs(["status"]);
		const result = await runPollinateCommand(inv, enqueuer, reader, sink);

		expect(result.exitCode).toBe(0);
		expect(lines.some((l) => l.includes("no pollinating pass on record"))).toBe(true);
	});
});
...
describe("c-AC-4: compaction completes → state updated, next pass is incremental", () => {
	it("the runner's stateUpdater fires once on a successful compaction pass", async () => {
		...
		const model = createFakeModelClient({ memory_pollinating: mutationBody });
	...
	it("an empty graph compaction (null payload) still calls stateUpdater → pending clears", async () => {
		...
		const { storage } = storageWith(graphResponder({ entities: [], attributes: [] }));
		const model = createFakeModelClient({ memory_pollinating: '{"mutations":[],"summary":""}' });
		const updater = new FakeStateUpdater();
		const strategy = new CompactionPayloadStrategy(128_000);
