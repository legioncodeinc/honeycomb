 ...
 *
 * ── THE FIX ─────────────────────────────────────────────────────────────────
 * `handleConversation` now applies an identity-bound cross-tenant guard (the same
 * one PR #156 swept across every scope resolver): when an authenticated Identity is
 * present on the context (team/hybrid mode), the resolved org MUST equal `identity.org`
 * and the workspace is taken from `identity.workspace`, not the header. A forged org
 * header that disagrees with `identity.org` fails closed with 400 ("org mismatch").
 *
...
describe("Cross-tenant guard for GET /api/hooks/conversation (pentest mitigation)", () => {
	...
	it("team mode: rejects a forged org header that disagrees with identity.org (400)", async () => {
		...
		// The handler rejects the forged org with 400 (identity-bound guard: the header
		// org disagrees with identity.org, so the read fails closed before any storage call).
		expect(res.status).toBe(400);
		const body = (await res.json()) as { error: string; reason: string };
		expect(body.error).toBe("bad_request");
		expect(body.reason).toContain("org mismatch");
