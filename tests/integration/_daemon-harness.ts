...
export interface BootTestDaemonOptions {
	 ...
	 * ENABLED pollinating pass reaches the real `memory_pollinating` model. Absent → the daemon's
	 * default (`agent.yaml` under the workspace root); unparseable → the no-op model client.
	 */
	readonly agentConfigPath?: string;
	/**
	 * The pollinating `memory.pollinating` config provider seam (PRD-026 AC-W gate). Forwarded
	 * verbatim to {@link assembleDaemon}. PRD-045d (d-AC-2) injects a fixed `{ enabled: true }`
	 * provider so the assembled gate STARTS the real pollinating worker WITHOUT mutating
	 * `process.env` (the worker is built + started by the composition root, not the test).
	 * Absent → the env provider (OFF unless `HONEYCOMB_POLLINATING_ENABLED`).
	 */
	readonly pollinatingConfigProvider?: Parameters<typeof assembleDaemon>[0]["pollinatingConfigProvider"];
}
...
export async function bootTestDaemon(options: BootTestDaemonOptions = {}): Promise<BootedTestDaemon> {
	...
	const assembled = assembleDaemon({
		...
		...(options.pollinatingConfigProvider !== undefined ? { pollinatingConfigProvider: options.pollinatingConfigProvider } : {}),
	});
