/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  LIVE POLLINATING CONSOLIDATION PROOF — PRD-026 Wave 2a (AC-3 / AC-4 / AC-5). ║
 * ╠══════════════════════════════════════════════════════════════════════════╣
 * ║  The behavioral bar that LICENSES flipping the shipped default OFF→ON: a    ║
 * ║  REAL pollinating pass, run END-TO-END through the daemon-resident worker      ║
 * ║  (enqueue → `worker.runOnce()`), against LIVE DeepLake with the REAL        ║
 * ║  Anthropic `memory_pollinating` model, CONSOLIDATES a deliberately-messy       ║
 ...
 * ║      table (`ci_pollinate_jobs_<run>`) it DROPs in afterAll — the same isolation ║
 * ║      seam `pollinating-counter-live.itest.ts` uses for `pollinating_state`.        ║
...
import { POLLINATING_JOB_KIND } from "../../src/daemon/runtime/pollinating/contracts.js";
import { createPollinatingTrigger } from "../../src/daemon/runtime/pollinating/trigger.js";
...
describe.skipIf(!GATED)("PRD-026 live pollinating consolidation proof (AC-3/AC-4/AC-5, gated)", () => {
	...
	beforeAll(async () => {
		...
		secretsBaseDir = mkdtempSync(join(tmpdir(), "hc-pollinate-secrets-"));
	...
	it(
		"a real pollinating pass consolidates the seeded graph without losing source-backed memory (AC-3/AC-4/AC-5)",
		async ({ skip }) => {
			...
			// NEUTRAL via a SKIP + the run-level sentinel rather than red-ing the pollinating-pass
			...
			await neutralizeIfInfraDegraded("pollinating-consolidation-live:preflight", () => storage.connect(scope), skip);
			...
			// This mirrors `assemble.ts`'s `buildGatedPollinatingWorker` EXACTLY: the real
			// inference model client (Anthropic `memory_pollinating`), the real trigger, the real
			// worker. The queue is pointed at a throwaway table for isolation.
			const pollinatingConfig = PollinatingConfigSchema.parse({ enabled: true, backfillOnFirstRun: true });
			...
			const trigger = createPollinatingTrigger({ storage, scope, config: pollinatingConfig, enqueuer: queue });
			const worker = createPollinatingWorker({ queue, storage, scope, config: pollinatingConfig, model, trigger });

			// Enqueue ONE pollinating job for THIS run's agent, then run exactly one pass.
			await queue.enqueue({
			...
			expect(processed, "the worker leased + ran the pollinating job").toBe(true);
