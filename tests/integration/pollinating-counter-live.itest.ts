/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  LIVE pollinating_state COUNTER SMOKE — OPT-IN, MUTATES A REAL DEEPLAKE BACKEND.║
 * ╠══════════════════════════════════════════════════════════════════════════╣
 * ║  THIS SUITE WRITES TO A REAL DEEPLAKE ORG. It lazily creates a            ║
 * ║  `pollinating_state` table from the catalog ColumnDef array, increments +     ║
 ...
 * ║      (`ci_pollinating_<run-id>`) via the `tableName` seam — the SAME ColumnDef  ║
 * ║      array, a DROP-able name — never a real daemon's `pollinating_state`. DROP  ║
 ...
 * transport in `tests/daemon/runtime/pollinating/trigger.test.ts`).
 */
...
const RUN_ID = runId();
const CI_TABLE = `ci_pollinating_${RUN_ID}`;
 ...
 * actually wrote.
 */
class RecordingQueue implements PollinatingJobEnqueuer {
...
describe.skipIf(!HAS_TOKEN)("live pollinating_state counter smoke (opt-in, real backend)", () => {
	let storage: StorageClient;
	let org: string;
	let workspace: string;
	const SCOPE_AGENT: PollinatingScope = { agentId: `ci-agent-${RUN_ID}` };
	...
	function triggerWith(tokenThreshold: number): PollinatingTrigger {
		return createPollinatingTrigger({
			storage,
			scope: { org, workspace },
			config: PollinatingConfigSchema.parse({ enabled: true, tokenThreshold }),
	...
	it("increment accumulates and the highest-version read returns it (a-AC-1 / a-AC-5)", async () => {
		...
		await trigger.incrementPollinatingCounter(SCOPE_AGENT, 40);
		await trigger.incrementPollinatingCounter(SCOPE_AGENT, 35);
	...
	it("threshold → enqueue + reset SUBTRACTS, and an interleaved write is not lost (a-AC-2 / FR-5)", async () => {
		...
		const scope: PollinatingScope = { agentId: `ci-agent-${RUN_ID}-b` };
		const queue = new RecordingQueue();
		const trigger = createPollinatingTrigger({
			storage,
			scope: { org, workspace },
			config: PollinatingConfigSchema.parse({ enabled: true, tokenThreshold: 100 }),
			enqueuer: queue,
			tableName: CI_TABLE,
		});

		// Accumulate 130 (over the 100 threshold).
		await trigger.incrementPollinatingCounter(scope, 130);
		// Interleave a summary write of 50 BEFORE the tick (the concurrent-write window).
		await trigger.incrementPollinatingCounter(scope, 50); // total now 180.

		const result = await trigger.checkAndEnqueuePollinating(scope);
		expect(result.decision).toBe("enqueued");
		// EXACTLY ONE enqueue across the threshold crossing (a-AC-2).
		expect(queue.calls).toHaveLength(1);

		// The enqueued job carries the right `{ mode, agentId, ... }` pollinating payload
		// (the runner reads these back off the queue, so the shape is load-bearing).
		const job = queue.calls[0];
		expect(job.kind).toBe("pollinating");
	...
	it("a 2nd tick while pending_job_id is set enqueues NOTHING (single-pending guard, a-AC-2 / a-AC-3)", async () => {
		...
		const scope: PollinatingScope = { agentId: `ci-agent-${RUN_ID}-c` };
		const queue = new RecordingQueue();
		const trigger = createPollinatingTrigger({
			storage,
			scope: { org, workspace },
		...
		await trigger.incrementPollinatingCounter(scope, 250); // 250 ≥ 100.
		const first = await trigger.checkAndEnqueuePollinating(scope);
		...
		const second = await trigger.checkAndEnqueuePollinating(scope);
