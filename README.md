...
<p align="center">
  <a href="https://theapiary.sh"><img src="assets/honeycomb-intro.gif" alt="Honeycomb in action — capture on one harness, recall on another" width="840"></a>
</p>
...
#### 🛹 For vibe coders
Stop re-explaining your project to a fresh agent every morning. Honeycomb remembers your decisions, your conventions, and the fixes that worked — and primes your next session with them automatically. One install command, a friendly dashboard, no SQL, no config gauntlet.
...
3. Done — the same running daemon lights up its Deep Lake-backed surfaces. Capture and recall go live.
...
- **Versioned & append-only.** Writes bump a version instead of mutating in place, so memory's full history stays on disk. Supersession marks old facts stale without losing them — that's what makes the pollinating loop safe and auditable.
