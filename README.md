...
- **TypeScript SDK** — the `@legioncodeinc/honeycomb` client with framework subpath entries (`@legioncodeinc/honeycomb/react`, `@legioncodeinc/honeycomb/vercel`, `@legioncodeinc/honeycomb/openai`). The core entry is fetch-only and browser-safe; `react` and `ai` are optional peers.
