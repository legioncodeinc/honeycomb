 ...
 *          name + the per-project state the daemon serves: bound path(s), git remote, last-capture, and
 *          memory/session counts (`scope/projects` aggregates these — paths/remote from the local store +
 *          registry, counts best-effort fail-soft to 0 on a backend flap). A field the daemon serves
 *          empty/absent (no path, no remote, no captures) renders as an honest "—" rather than a fabricated value.
 ...
 * the daemon serves empty/absent renders as an honest "—" rather than a fabricated value. Per-row OPEN
