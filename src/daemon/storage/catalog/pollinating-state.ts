/**
 * PRD-009a — Pollinating token-budget counter catalog table (`pollinating_state`).
 *
 * The single `USING deeplake` table that backs the pollinating loop's token-budget
 ...
 * of an in-flight pollinating pass. Written ONLY by the daemon's maintenance loop
 * through the PRD-002d write primitives + the PRD-002c heal engine — never a
 * hand-rolled `ALTER` (FR-8). The trigger module (`runtime/pollinating/trigger.ts`)
 ...
 * daemon-internal queue keyed only by job id), `pollinating_state` IS agent-scoped
...
/** The bare `pollinating_state` table name, exported so writers never re-spell it. */
export const POLLINATING_STATE_TABLE = "pollinating_state" as const;

/**
 * `pollinating_state` — the per-scope token-budget counter (FR-1). Column order:
 ...
 * - `pending_job_id`         the `memory_jobs` id of the in-flight pollinating pass,
 ...
 */
export const POLLINATING_STATE_COLUMNS = Object.freeze([
...
/** The 009a pollinating-state group — spread into `CATALOG` by the barrel. */
export const POLLINATING_STATE_TABLES: readonly CatalogTable[] = defineGroup([
	{
		name: POLLINATING_STATE_TABLE,
		columns: POLLINATING_STATE_COLUMNS,
