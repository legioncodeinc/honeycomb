/**
 * Pollinating token-budget trigger — PRD-009a (IMPLEMENTED, Wave 1).
 *
 * The `pollinating_state` counter lifecycle: increment on every session-summary write
 * (FR-2), compare against the threshold on each maintenance tick (FR-3), and at the
 * threshold — with no pass already pending — enqueue exactly ONE pollinating job,
 ...
 * Every increment and every reset APPENDs a new `pollinating_state` row at `version` =
 ...
 * `checkAndEnqueuePollinating` reads the current counter, enqueues, then appends a
...
import { POLLINATING_STATE_TABLE, healTargetFor } from "../../storage/catalog/index.js";
...
import { POLLINATING_JOB_KIND, type PollinatingPassMode } from "./contracts.js";
 ...
 */
export interface PollinatingScope {
 ...
 */
export interface PollinatingState {
	...
	/** The in-flight pollinating job's id, or empty when none is pending (FR-6). */
 ...
 */
export interface PollinatingJobEnqueuer {
...
export interface PollinatingClock {
	/** Current wall-clock time in ms (defaults to `Date.now`). */
	readonly now: () => number;
}

/** The default clock: real `Date.now`. */
function defaultClock(): PollinatingClock {
	return { now: () => Date.now() };
}

/**
 * Derive the deterministic `pollinating_state` row id for a scope (D-1). Keyed on the
 ...
 * `pollinate_`. Pure.
 */
export function pollinatingStateId(scope: PollinatingScope): string {
	const hash = crypto.createHash("sha256").update(`pollinating:${scope.agentId}`).digest("hex").slice(0, 24);
	return `pollinate_${hash}`;
}
...
/** Construction deps for the pollinating trigger. */
export interface PollinatingTriggerDeps {
	/** Run queries through this — never a raw fetch. */
	readonly storage: StorageQuery;
	/** The resolved `{ org, workspace }` partition for counter rows. */
	readonly scope: QueryScope;
	/** The resolved `memory.pollinating` config. */
	...
	readonly clock?: PollinatingClock;
	 ...
	 * `pollinating_state`. Exists so the opt-in LIVE counter smoke can point the trigger
	 * at a throwaway, namespaced table it is free to DROP, WITHOUT touching a real
	 * daemon's shared `pollinating_state`. Validated through `sqlIdent`; the COLUMNS are
	 * always the catalog's single-sourced `POLLINATING_STATE_COLUMNS`.
	 */
	readonly tableName?: string;
}

/**
 * The pollinating trigger. Construct via {@link createPollinatingTrigger} with the storage
 * client, the resolved partition scope, the pollinating config, and the job enqueuer.
...
export class PollinatingTrigger {
	...
	constructor(deps: PollinatingTriggerDeps) {
		...
		// Defaults to the catalog's canonical `pollinating_state`; the opt-in LIVE counter
		...
		// `POLLINATING_STATE_COLUMNS`, only the name is parameterized.
		this.tableName = sqlIdent(deps.tableName ?? POLLINATING_STATE_TABLE);
		this.target = { table: this.tableName, columns: healTargetFor(POLLINATING_STATE_TABLE).columns };
	}
	 ...
	 * the backend's segment-freshness flap. Returns a zero-version {@link PollinatingState}
	 * when the scope has no row yet. The `agent_id` conjunct keeps the read inside the
	 * scope's agent ring (a-AC-6).
	 */
	async readState(scope: PollinatingScope): Promise<PollinatingState> {
		const id = pollinatingStateId(scope);
		let best: PollinatingState | null = null;
	 ...
	 * in-place UPDATE. Heal-aware (the first write to a missing `pollinating_state` table
	 * CREATEs it). Returns the new version.
	 */
	private async appendVersion(
		state: PollinatingState,
	 ...
	 */
	async incrementPollinatingCounter(scope: PollinatingScope, tokens: number): Promise<number> {
	 ...
	 * unchanged. Append-only version-bump (the SAME path `checkAndEnqueuePollinating`'s
	 * terminal-clear takes), never an in-place UPDATE.
	 *
	 * This is the seam the pollinating RUNNER's {@link PollinatingStateUpdater} adapter calls
	 ...
	 * ring (a-AC-6).
	 */
	async recordPassComplete(scope: PollinatingScope, passAt: string): Promise<void> {
		const state = await this.readState(scope);
		await this.appendVersion(state, scope, {
	 ...
	 *   2. If a pass is already pending ({@link PollinatingState.pendingJobId} non-empty),
	 ...
	 *      counter keeps growing via {@link incrementPollinatingCounter}, but no enqueue.
	 *   4. If `tokensSinceLastPass < tokenThreshold` → return `below_threshold`.
	 *   5. Otherwise ENQUEUE exactly one pollinating job, then append a reset version that
	 ...
	 */
	async checkAndEnqueuePollinating(
		scope: PollinatingScope,
		mode: PollinatingPassMode = "incremental",
	): Promise<PollinatingTickResult> {
		const state = await this.readState(scope);
...
export type PollinatingTickDecision = "enqueued" | "skipped" | "disabled" | "below_threshold";

/** Why a tick reached its decision (for diagnostics + tests). */
export type PollinatingTickReason =
	| "threshold-met"
	| "pending"
	| "pending-cleared"
	| "disabled"
...
export interface PollinatingTickResult {
	/** What the tick decided. */
	readonly decision: PollinatingTickDecision;
	/** Why it decided that. */
	readonly reason: PollinatingTickReason;
...
/** Build a {@link PollinatingTrigger}. The daemon injects the real deps; tests inject fakes. */
export function createPollinatingTrigger(deps: PollinatingTriggerDeps): PollinatingTrigger {
	return new PollinatingTrigger(deps);
}
