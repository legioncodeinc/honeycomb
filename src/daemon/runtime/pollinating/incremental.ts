/**
 * 009b — Incremental pollinating payload strategy + the on-demand graph-query tool.
 *
 * ════════════════════════════════════════════════════════════════════════════
 ...
 * {@link PollinatingRunner} harness owns the model call, the 008c apply loop, and the
 * state update. Do NOT edit any other pollinating file.
 * ════════════════════════════════════════════════════════════════════════════
 *
 ...
 *
 *   1. resolve the scope's `last_pass_at` (highest-version `pollinating_state` read,
 ...
 *      pollinating sessions + `MEMORY.md`) + the pollinating-only `POLLINATING.md` task prompt
 ...
 * `null` short-circuit) — an incremental pass with nothing to pollinate over is a no-op,
 * not a wasted completion.
 *
 * ── The identity / file seam (FR-1 / FR-2 / b-AC-1) ──────────────────────────
 * The startup identity files, prior pollinating sessions, `MEMORY.md`, and the
 * `POLLINATING.md` task prompt are FILE/SEAM sources. Their real wiring is the identity
 * preset (PRD-019) + capture (PRD-005); this module takes them through the injected
 * {@link PollinatingIdentitySource} so the payload SHAPE is correct today and the daemon
 * swaps in the real file loader at assembly. The default source yields empties EXCEPT
 * a built-in {@link DEFAULT_POLLINATING_TASK_PROMPT} — the load-bearing 009b invariant is
 * that `POLLINATING.md` is present ONLY for a pollinating pass and never in normal startup
 * (FR-2), which the seam enforces structurally: nothing outside pollinating calls it.
 *
...
import { POLLINATING_STATE_TABLE } from "../../storage/catalog/index.js";
...
import { pollinatingStateId } from "./trigger.js";
...
/**
 * The identity-preset payload a pollinating pass prepends (FR-1 / FR-2 / b-AC-1).
 * These are FILE/SEAM sources owned elsewhere (identity preset PRD-019, capture
 * PRD-005); 009b consumes them through {@link PollinatingIdentitySource} so the payload
 * SHAPE is fixed now and the real file wiring lands at daemon assembly.
 */
export interface PollinatingIdentityContext {
	/** The startup identity files (persona/system context) loaded at session start. */
	readonly identityFiles: readonly string[];
	/** Prior pollinating-session transcripts/summaries so the pass sees its own decisions. */
	readonly priorPollinatingSessions: readonly string[];
	/** `MEMORY.md` — the agent's curated memory doc. */
	readonly memoryMd: string;
	/**
	 * The `POLLINATING.md` task prompt — loaded ONLY for a pollinating pass and NEVER in
	 ...
	 */
	readonly pollinatingTaskPrompt: string;
}

/**
 * The seam that yields the {@link PollinatingIdentityContext} for a pass's scope/agent.
 ...
 */
export interface PollinatingIdentitySource {
	/** Load the identity preset + `POLLINATING.md` for `scope`/`job` (FR-1 / FR-2). */
	load(scope: QueryScope, job: PollinatingJobPayload): Promise<PollinatingIdentityContext>;
}

/**
 * The built-in `POLLINATING.md` task prompt the default source supplies (FR-2). The real
 * `POLLINATING.md` file ships with the identity preset (PRD-019); this is the in-code
 * default so a pass running before that wiring still carries the pollinating-only task
 * framing. It is pollinating-only by construction — only the incremental strategy reads it.
 */
export const DEFAULT_POLLINATING_TASK_PROMPT =
	"# POLLINATING\n" +
	"You are running a corrective-maintenance (pollinating) pass over the knowledge graph. " +
 ...
 * {@link DEFAULT_POLLINATING_TASK_PROMPT}. Used until the identity-preset wiring (PRD-019)
 * injects the real file loader — the payload SHAPE is correct and the pollinating-only
 * `POLLINATING.md` invariant holds.
 */
export function defaultPollinatingIdentitySource(): PollinatingIdentitySource {
	return {
		load(): Promise<PollinatingIdentityContext> {
			return Promise.resolve({
				identityFiles: [],
				priorPollinatingSessions: [],
				memoryMd: "",
				pollinatingTaskPrompt: DEFAULT_POLLINATING_TASK_PROMPT,
			});
		},
	};
}
...
/** Poll budget for the highest-version `pollinating_state` read (flap-robust, like the trigger). */
...
/**
 * Resolve a scope's `last_pass_at` from the highest-version `pollinating_state` row
...
async function readLastPassAt(storage: StorageQuery, scope: QueryScope, agentId: string): Promise<string> {
	const id = pollinatingStateId({ agentId });
	const tbl = sqlIdent(POLLINATING_STATE_TABLE);
 ...
 * section would exceed `tokenBudget` (D-2). Identity + POLLINATING.md lead (they frame
...
export interface IncrementalStrategyDeps {
	...
	/** The identity / file seam (FR-1 / FR-2); defaults to {@link defaultPollinatingIdentitySource}. */
	readonly identitySource?: PollinatingIdentitySource;
 ...
 * pollinating-only `POLLINATING.md` task prompt + the graph-query-tool affordance — capped
 * to `maxInputTokens`, and returns `null` when there are no new summaries to pollinate
...
export class IncrementalPayloadStrategy implements PollinatingPayloadStrategy {
	readonly mode: PollinatingPassMode = "incremental";

	private readonly maxInputTokens: number;
	private readonly identitySource: PollinatingIdentitySource;
	...
	constructor(deps: IncrementalStrategyDeps = {}) {
		...
		this.identitySource = deps.identitySource ?? defaultPollinatingIdentitySource();
	 ...
	 * delta; loads the identity preset + `POLLINATING.md`; caps to `maxInputTokens`.
	 */
	async loadPayload(
		storage: StorageQuery,
		scope: QueryScope,
		...
		// 1. Resolve the delta boundary (highest-version pollinating_state read, scoped).
		...
		if (newSummaries.length === 0) {
			// Nothing to pollinate over — the harness records an empty pass, never calls the model.
			return null;
		}
		...
		// 4. The identity preset + the pollinating-only POLLINATING.md task prompt (FR-1 / FR-2).
		const identity = await this.identitySource.load(scope, job);

		const sections: PromptSection[] = [
			{ heading: "POLLINATING.md (task)", body: identity.pollinatingTaskPrompt },
			{ heading: "Identity files", body: identity.identityFiles.join("\n\n") },
			{ heading: "MEMORY.md", body: identity.memoryMd },
			{ heading: "Prior pollinating sessions", body: identity.priorPollinatingSessions.join("\n\n") },
			{
 ...
 * resolved `memory.pollinating` config) + the identity-file seam.
 */
export function createIncrementalStrategy(deps: IncrementalStrategyDeps = {}): PollinatingPayloadStrategy {
