# Pollinating module — CONVENTIONS (PRD-009)

The pollinating loop lives under `src/daemon/runtime/pollinating/` (daemon-only; the
DeepLake path lives only in the daemon bundle — the `tests/daemon/storage/invariant.test.ts`
import-graph assertion enforces it). Wave 1 built the `pollinating_state` catalog
table, the pollinating config, the shared contracts, 009a (the token-budget trigger,
DONE), and the pollinating session-runner HARNESS, and pre-wired the 009b / 009c
...
|---|---|---|
| `config.ts` | `memory.pollinating` config (zod): `enabled`, `tokenThreshold` (100k), `maxInputTokens` (128k), `backfillOnFirstRun`. `resolvePollinatingConfig` + the env provider. | DONE |
| `contracts.ts` | The cross-module shapes: `PollinatingJobPayload`, the seven-op mutation vocabulary `POLLINATING_MUTATION_KINDS` + `MUTATION_KIND_TO_OPERATION`, `PollinatingMutation` / `PollinatingMutationSet` (zod boundaries), `PollinatingPassResult`. The `parse*` boundary validators. A genuinely new cross-module field is a Wave-1 change (raise it), not a stub edit. | DONE |
| `trigger.ts` | 009a IN FULL — `PollinatingTrigger` with `incrementPollinatingCounter` (FR-2) + `checkAndEnqueuePollinating` (FR-3..7). Append-only version-bumped; reset SUBTRACTS the threshold; the single-pending guard. | DONE |
| `runner.ts` | The pass-lifecycle HARNESS: the `PollinatingPayloadStrategy` seam, the model call (`memory_pollinating` workload), the defensive parse, the 008c apply loop, the state update on success. | DONE (harness) |
| `../../storage/catalog/pollinating-state.ts` | The `pollinating_state` catalog table (version-bumped, `scope:"agent"`, FR-1 columns). | DONE |
...
- **D-2** thresholds 100k / 128k, configurable under `memory.pollinating`.
- **D-3** `pollinating_state` is APPEND-ONLY version-bumped (highest-version read); the
...
export class IncrementalPayloadStrategy implements PollinatingPayloadStrategy {
  readonly mode = "incremental" as const;            // 009c: "compaction"
  async loadPayload(storage, scope, job): Promise<PollinatingPayload | null> {
    // assemble the prompt + token budget for this scope/mode; return null when
    // there is nothing to pollinate over (the harness records an empty pass).
  }
}
```

`PollinatingPayload` is `{ prompt: string; tokenBudget: number }`. The harness forwards
`prompt` to the model and echoes `tokenBudget` for accounting — it is blind to HOW
you assembled it. Return `null` to mean "nothing to pollinate over" (no new summaries
for incremental; empty graph for compaction).

## The runner harness you CONSUME (don't re-implement)

`PollinatingRunner.runPass(job)` already does ALL of this — you supply only the strategy:

1. `strategy.loadPayload(...)` → `null` short-circuits to an empty pass.
2. `model.complete("memory_pollinating", payload.prompt)` — the STRONGER target (D-5 /
   b-AC-6). The model seam is raw-in/raw-out; the harness strips CoT + a fence,
   JSON-parses, and validates via `parsePollinatingMutationSet` (drop-invalid, never
   fails the job).
...
(and, for 009c, the `honeycomb pollinate` CLI verb).
...
The model returns a `PollinatingMutationSet`: `{ mutations: PollinatingMutation[], summary,
tokenBudget }`. Each `PollinatingMutation` is `{ kind, payload, rationale, confidence,
...
|---|---|---|
| 009b incremental runner | `pollinating/incremental.ts` | `tests/daemon/runtime/pollinating/incremental.test.ts` |
| 009c compaction + CLI | `pollinating/compaction.ts` | `tests/daemon/runtime/pollinating/compaction.test.ts` |

The 009a trigger test is `tests/daemon/runtime/pollinating/trigger.test.ts` (DONE).
...
FAKE transport + a FAKE `ModelClient` (`createFakeModelClient({ memory_pollinating:
...
- Resolve a `HealTarget` via `healTargetFor("pollinating_state")` from
...
`readState` already does this for `pollinating_state`; the job queue does it for
...
PRD-009 left a gap: nothing in the LIVE daemon CONSUMES a `pollinating` job. The trigger
enqueues and the runner can run, but no harness leased a `pollinating` job and invoked the
runner. PRD-026 Wave 1 Track B fills that with `worker.ts` — `createPollinatingWorker(deps)`
→ a `PollinatingJobWorker` modelled on `pipeline/stage-worker.ts` (same `runOnce()` /
`start()` / `stop()` shape, overlap guard, injected `setTimer`/`clearTimer`).

`runOnce()`:

1. `queue.lease(["pollinating"])` — the additive kind filter (below). `null` → return false.
2. `parsePollinatingJobPayload(leased.payload)`. A malformed payload → `queue.fail(id, …)`,
...
3. Select the strategy by mode (D-4): `compaction` when `mode === "compaction"` OR the
   ...
   `maxInputTokens` is threaded from the resolved `memory.pollinating` config.
...
5. On success `queue.complete(id)`; on throw `queue.fail(id, message)` — the stage-worker
   try/catch shape (`pollinating.worker.*` events), never a swallowed error.
...
ANY kind (existing callers unchanged); supplying `["pollinating"]` leases ONLY a pollinating
...
The runner's `PollinatingStateUpdater.recordPassComplete(agentId, passAt)` is the b-AC-5
seam. The worker builds it from the trigger via a NARROW, ADDITIVE public method added to
`PollinatingTrigger`: `recordPassComplete(scope, passAt)` — it reads the current state and
...
never a new in-place write. The worker's `PollinatingTriggerSeam` dep exposes just
`readState` + `recordPassComplete`; the worker adapts `(agentId, passAt)` onto
`(scope, passAt)`.

## Enablement — operator guide (PRD-026, D-1)

The pollinating loop is a PREMIUM tier: it makes real model calls (the `memory_pollinating`
...
HONEYCOMB_POLLINATING_ENABLED=false   # or simply unset

# On — the gated worker is built + started at assembly (it leases ONLY ["pollinating"]),
...
|---|---|---|
| `HONEYCOMB_POLLINATING_ENABLED` | `false` | Master switch. OFF → counter grows, nothing queued, worker not built. |
| `HONEYCOMB_POLLINATING_TOKEN_THRESHOLD` | `100000` | Tokens-since-last-pass that queues a pass; the reset SUBTRACTS this (FR-5). |
| `HONEYCOMB_POLLINATING_MAX_INPUT_TOKENS` | `128000` | Input budget a pass's payload is sampled to (compaction). |
| `HONEYCOMB_POLLINATING_BACKFILL_ON_FIRST_RUN` | `true` | First run with no prior pass enters compaction (full graph) not incremental. |

When ON, the real pass ALSO needs the inference model wired: an `agent.yaml`
`inference:` block at the workspace root with the `memory_pollinating` workload, plus the
...
`buildInferenceModelClient` degrades to the no-op client and pollinating yields empty,
...
```bash
# Enqueue a pass on the loopback daemon (the diagnostics "Pollinate now" seam).
honeycomb pollinate trigger

# Ask for a full-graph COMPACTION pass (vs the steady-state incremental).
honeycomb pollinate trigger --compact
```

`honeycomb pollinate trigger` POSTs to `POST /api/diagnostics/pollinate` through the loopback
...
off, pointing you back at `HONEYCOMB_POLLINATING_ENABLED=true`). The ack carries NO token
or secret (AC-6).
...
Flipping the SHIPPED default ON (so a fresh install pollinates without an explicit knob) is
licensed ONLY by the AC-5 behavioral proof:
`tests/integration/pollinating-consolidation-live.itest.ts` — a real pass, against live
...
the worker ONLY when `resolvePollinatingConfig().enabled` (default OFF), stop it in
teardown, and wire the trigger's `pendingTerminal` probe + the maintenance-loop tick /
summary-writer counter calls. `createPollinatingWorker` does NOT decide enablement.
