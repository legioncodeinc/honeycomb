/**
 * 009c — Compaction dreaming payload strategy + mode-selection helper.
 *
 * Implements {@link CompactionPayloadStrategy}: the full-graph pass strategy the
 * {@link DreamingRunner} harness injects for a first-run backfill or an on-demand
 ...
 * when `backfillOnFirstRun` is true (c-AC-1); `honeycomb dream trigger --compact`
...
const COMPACTION_SYSTEM = `You are Honeycomb's compaction dreaming agent. You reason over the FULL entity graph and a sample of recent summaries to propose structural cleanup: merge duplicates, archive junk entities and claims, fix stale attributes.
...
/**
 * Determine whether a dreaming pass should enter compaction mode instead of
 ...
 *
 * Callers: the trigger's `checkAndEnqueueDreaming` and the daemon assembly's
 * mode-selection step BOTH consult this helper. It is pure (no I/O).
 */
export function shouldEnterCompaction(config: DreamingConfig, lastPassAt: string): boolean {
 ...
 */
export function resolvePassMode(config: DreamingConfig, lastPassAt: string): DreamingPassMode {
 ...
 */
export class CompactionPayloadStrategy implements DreamingPayloadStrategy {
	readonly mode: DreamingPassMode = "compaction";
 ...
 */
export function createCompactionStrategy(maxInputTokens?: number): DreamingPayloadStrategy {
