/**
 * Dreaming-loop contracts — PRD-009 Wave 1 (the typed shapes 009a/009b/009c share).
 *
 * The cross-module data contracts for the dreaming loop: the queued dreaming-job
 * payload (what 009a enqueues into `memory_jobs` and 009b/009c consume), the
 * mutation set the dreaming model returns (the human-facing op vocabulary, mapped
 ...
 * zod validates at the UNTRUSTED boundaries: the dreaming-job payload parsed back
 * off the `memory_jobs` queue, and the mutation set returned by the model (raw
 * text → parsed JSON → validated set). The runner builds the {@link DreamingPassResult}
 ...
 * The model emits the seven-op set below ({@link DREAMING_MUTATION_KINDS}); the
...
/**
 * The two dreaming pass modes (D-4):
 ...
 */
export const DREAMING_PASS_MODES = Object.freeze(["incremental", "compaction"] as const);
/** A dreaming pass mode. */
export type DreamingPassMode = (typeof DREAMING_PASS_MODES)[number];

// ────────────────────────────────────────────────────────────────────────────
// Dreaming-job payload (FR-4 / 009b/009c) — the queued job body. Boundary: zod.
// ────────────────────────────────────────────────────────────────────────────

/** The job kind 009a enqueues into `memory_jobs` (routes to the dreaming runner). */
export const DREAMING_JOB_KIND = "dreaming" as const;

/**
 * The payload of a queued dreaming job (FR-4). 009a builds it at enqueue; 009b/009c
 * parse it back off the queue via {@link parseDreamingJobPayload}. Carries the pass
 ...
 */
export const DreamingJobPayloadSchema = z.object({
	/** Incremental (post-last-pass delta) or compaction (full graph). */
	mode: z.enum(DREAMING_PASS_MODES).default("incremental"),
...
/** A validated dreaming-job payload (the queue boundary). */
export type DreamingJobPayload = z.infer<typeof DreamingJobPayloadSchema>;

/**
 * Validate a candidate dreaming-job payload at the queue boundary, returning the
 * typed {@link DreamingJobPayload} or `null` on an invalid body. Drop-invalid
 * (never throw) so a malformed queue row is rejected without crashing the runner.
 * Tolerates extra keys.
 */
export function parseDreamingJobPayload(candidate: unknown): DreamingJobPayload | null {
	const parsed = DreamingJobPayloadSchema.safeParse(candidate);
...
/**
 * The seven dreaming-mutation kinds the model emits (009b FR-6). This is the
 ...
 */
export const DREAMING_MUTATION_KINDS = Object.freeze([
	"create_entity",
	"merge_entities",
	"delete_entity",
	"update_aspect",
...
/** A dreaming-mutation kind (the model-facing op vocabulary). */
export type DreamingMutationKind = (typeof DREAMING_MUTATION_KINDS)[number];

/**
 * Map a dreaming-mutation kind onto the 008c control-plane operation the runner
 ...
 */
export const MUTATION_KIND_TO_OPERATION: Readonly<Record<DreamingMutationKind, ProposalOperation>> = Object.freeze({
...
/**
 * One mutation the dreaming model emits (009b FR-6). Carries the `kind` (the
 ...
 * via {@link DreamingMutationSchema} before the runner submits it.
 */
export const DreamingMutationSchema = z.object({
	/** The human-facing mutation kind (mapped to a control-plane operation). */
	kind: z.enum(DREAMING_MUTATION_KINDS),
...
/** A validated dreaming mutation (the model boundary). */
export type DreamingMutation = z.infer<typeof DreamingMutationSchema>;

/**
 * The full mutation set the dreaming model returns (009b FR-6): the ordered
 ...
 * {@link DreamingMutationSetSchema}.
 */
export const DreamingMutationSetSchema = z.object({
	/** The ordered mutations to apply through the control plane. */
	mutations: z.array(DreamingMutationSchema).default([]),
...
/** A validated dreaming mutation set (the model boundary). */
export type DreamingMutationSet = z.infer<typeof DreamingMutationSetSchema>;

/**
 * Validate a candidate mutation set at the model boundary, returning the typed
 * {@link DreamingMutationSet} or `null` on an invalid body. Drop-invalid (never
 ...
 */
export function parseDreamingMutationSet(candidate: unknown): DreamingMutationSet | null {
	const parsed = DreamingMutationSetSchema.safeParse(candidate);
	return parsed.success ? parsed.data : null;
}
...
export interface DreamingMutationOutcome {
	/** The mutation that was submitted. */
	readonly kind: DreamingMutationKind;
...
/**
 * The result of a completed dreaming pass (009b). The runner builds this from
