/**
 * `honeycomb pollinate` CLI surface — PRD-009c FR-2 / c-AC-2.
 *
 * Two sub-commands:
 *   - `pollinate trigger --compact [--org O --workspace W --agent A]`
 ...
 *   - `pollinate status [--org O --workspace W --agent A]`
 *       Report the scope's last pollinating pass and pending-job state.
 *
 ...
 * performed through the injected {@link PollinateJobEnqueuer} seam — the daemon
 * assembly wires the real job-queue service; the AC-named test injects a fake.
 * State is read through the injected {@link PollinateStateReader} seam.
 *
 ...
 * sets `pending_job_id` on the pollinating_state row, so the single-pending guard
...
export interface PollinateScope {
	readonly org: string;
 ...
 * `PollinatingJobEnqueuer` (the PRD-004b queue). The test injects a fake that records
 * calls. Kept narrow (just `enqueue`) so the CLI holds no queue lifecycle knowledge.
 */
export interface PollinateJobEnqueuer {
	/** Enqueue a compaction pollinating job for the target scope; returns the job id. */
	enqueueCompaction(scope: PollinateScope): Promise<string>;
}

/** A snapshot of a scope's current pollinating state (for `pollinate status`). */
export interface PollinateStateSnapshot {
...
/**
 * The state-reader seam (for `pollinate status`). Returns `null` when no
 * pollinating_state row exists yet for the scope.
 */
export interface PollinateStateReader {
	readState(scope: PollinateScope): Promise<PollinateStateSnapshot | null>;
}

/** A line-sink so the command's output is capturable in tests (no direct stdout). */
export interface PollinateOutputSink {
	(line: string): void;
}

/** The parsed `pollinate` invocation: the sub-command + flags + the scope. */
export interface PollinateInvocation {
	/** The sub-command word ("trigger" | "status" | unknown). */
	readonly subCommand: string;
	/** `--compact` present (trigger only). */
	readonly compact: boolean;
	/** The resolved scope (org / workspace / agentId). */
	readonly scope: PollinateScope;
}

/** Outcome of running a `pollinate` command. */
 ...
 * {@link PollinateInvocation}. Scope flags default to empty org / default workspace /
 ...
 */
export function parsePollinateArgs(argv: readonly string[]): PollinateInvocation {
	let subCommand = "";
	let compact = false;
	let org = "";
	let workspace = "";
...
/**
 * Run a parsed `pollinate` command (c-AC-2 / FR-2). Two sub-commands:
 *
 ...
 */
export async function runPollinateCommand(
	inv: PollinateInvocation,
	enqueuer: PollinateJobEnqueuer,
	reader: PollinateStateReader,
	out: PollinateOutputSink,
): Promise<PollinateResult> {
	const { subCommand, compact, scope } = inv;

	if (subCommand === "trigger") {
		if (!compact) {
			out("error: `pollinate trigger` requires --compact.");
			out("       The automatic threshold trigger runs inside the daemon maintenance loop.");
			out("       To force a full-graph compaction pass: honeycomb pollinate trigger --compact");
	...
	if (subCommand === "status") {
		...
		out(`scope: org=${scope.org} workspace=${scope.workspace} agent=${scope.agentId}`);
		if (state === null) {
			out("status: no pollinating pass on record for this scope.");
			return { exitCode: 0, jobId: "" };
		}
	...
	out("usage: honeycomb pollinate trigger --compact [--org O --workspace W --agent A]");
	out("       honeycomb pollinate status [--org O --workspace W --agent A]");
	return { exitCode: subCommand === "" ? 0 : 1, jobId: "" };
}

/**
 * Convenience entry: parse + run a `pollinate` argv tail in one call (FR-2 / c-AC-2).
 * The daemon assembly step (deferred) injects the real enqueuer + reader; the
 * AC-named test injects fakes directly.
 */
export async function pollinateMain(
	argv: readonly string[],
	enqueuer: PollinateJobEnqueuer,
	reader: PollinateStateReader,
	out: PollinateOutputSink,
): Promise<PollinateResult> {
	return runPollinateCommand(parsePollinateArgs(argv), enqueuer, reader, out);
}
