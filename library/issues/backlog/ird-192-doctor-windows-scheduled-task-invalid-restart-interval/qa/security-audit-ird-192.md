# Security Audit — IRD-192 (Doctor Windows Scheduled Task Restart Interval)

| Field | Value |
|---|---|
| **Audit** | IRD-192 — `ird/192-doctor-windows-task-fix` |
| **Bee** | `security-worker-bee` |
| **Date** | 2026-06-30 |
| **Scope** | `git diff HEAD` working-tree changes (13 files): `doctor/src/cli/{context,dispatch,index,service-stub}.ts`, `doctor/src/service/{index,templates}.ts`, the 6 test files, and `scripts/install/install.{ps1,sh}` |
...
into `doctor status`. **None of the changed lines touch the command-launch path, the XML
...
target is the **`doctor/` subpackage** — a local-only TS/Node watchdog CLI with **no network
...
wires `serviceStateAsync → serviceStatus()` for `doctor status`.
...
- The structured-result refactor is **type-only** at the command boundary. The actual command
  ...
  (`doctor/src/service/index.ts:117`), passing **argv arrays**, not a concatenated string.
- `createExecFileRunner()` (`doctor/src/rungs/command-runner.ts`) wraps `node:child_process.
...
- The new `serviceStateAsync` wiring reuses the **identical** no-shell runner with the **identical**
  bounded timeout (`SERVICE_COMMAND_TIMEOUT_MS = 15_000`, `doctor/src/service/index.ts:43,253`).
  `serviceStatus()` (`doctor/src/service/index.ts:241-265`) runs `statusCommand(p, uid)` — a
...
**Verification.**
- `WINDOWS_RESTART_INTERVAL = "PT1M" as const` (`doctor/src/service/templates.ts:41`) is a
...
**Verification.**
- The 7 `message:` strings (`doctor/src/service/index.ts:150,169,183,190,201,224,230`) interpolate
...
- **Parity with pre-diff behavior:** the old code returned the same strings (e.g. the HEAD version
  of `index.ts:148` was ``return `Could not register Doctor service: ${error instanceof Error ?
...
prints `"... Run 'doctor install-service' to see why."`.
...
The changed code is entirely within `doctor/` (a local loopback watchdog CLI) plus two install
...
- **No captured-trace PII** — `doctor` does not read or write `sessions`/`memory` tables.
...
- `git diff HEAD --stat` reviewed: 13 files, +272/−60, confined to `doctor/src/{cli,service}/`,
  `doctor/tests/`, and `scripts/install/`. No unrelated changes.
