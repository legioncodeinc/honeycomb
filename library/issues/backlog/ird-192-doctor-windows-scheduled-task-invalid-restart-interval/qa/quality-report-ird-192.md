# Quality Report — IRD-192: Doctor Windows Scheduled Task invalid restart interval

> **Branch:** `ird/192-doctor-windows-task-fix`
> **Source plan:** `library/issues/backlog/ird-192-doctor-windows-scheduled-task-invalid-restart-interval/ird-192-doctor-windows-scheduled-task-invalid-restart-interval-index.md`
...
**Gate:** `cd doctor && npm run ci` → **GREEN**, typecheck clean, **493/493 tests passing (49 files)**.
...
| **AC-3** `schtasks /Run` → one Doctor process, status page binds 127.0.0.1:3852 | ✅ VERIFIED (unit) / LIVE-BLOCKED | `installCommands` includes `/Run /TN Doctor` (argv.test.ts). The process/bind check requires a live registered task (blocked by AC-1 env). |
| **AC-4** Doctor exits → Task Scheduler restarts within window | ✅ VERIFIED (design) / LIVE-BLOCKED | `RestartOnFailure` now carries valid `<Interval>PT1M</Interval>` + `<Count>999</Count>` (AC-2). Crash-recovery depends on a registered task (blocked by AC-1 env). |
| **AC-5** `doctor status` reports non-`unknown` service state for a registered task | ✅ VERIFIED | `cli/index.ts` `serviceStateAsync: () => serviceStatus({execPath, preferSystemScope, runner})`; `dispatch.ts runStatus` awaits it when present; `context.ts` adds the optional `serviceStateAsync` dep. `serviceStatus()` (service/index.ts:218) classifies via the real manager query, bounded by `SERVICE_COMMAND_TIMEOUT_MS`. A registered task resolves `running`, not `unknown`. |
...
| **AC-7** Installer does not print "Doctor is watching" on registration failure; prints non-fatal warning naming `doctor install-service` | ✅ VERIFIED | `install.ps1` + `install.sh` failure branches now append `Run 'doctor install-service' to see why.`; success "watching" line was already gated on exit code (unchanged) and now triggers correctly since AC-6 made the exit honest. Non-fatal (no hard `exit`/`return`). |
...
5. ✅ **Service status wired into `doctor status`** — bounded async probe; fast + fail-safe.
...
7. ⏳ **Release as Doctor patch** — version bump is Wave 3 (post-QA), not yet applied at QA time.

## Files-touched coverage (IRD "Files Touched")

| File in IRD | Changed? | Note |
|---|---|---|
| `doctor/src/service/templates.ts` | ✅ | `PT1M` constant + render. |
| `doctor/src/service/argv.ts` | ⚪ no change needed | argv already correct (`/Create /XML`, `/F`, `/Run`); IRD listed it speculatively. |
| `doctor/src/service/index.ts` | ✅ | structured result. |
| `doctor/src/cli/dispatch.ts` | ✅ | exit-code mapping. |
| `doctor/src/cli/index.ts` | ✅ | `serviceStateAsync` wiring. |
| `doctor/tests/service/templates.test.ts` | ✅ | AC-2 + AC-10 assertions. |
| `doctor/tests/service/service-module.test.ts` | ✅ | structured-result + AC-6 Windows scenario. |
| `doctor/tests/service/cli-delegation.test.ts` | ✅ | AC-6 exit-code test. |
| `scripts/install/install.ps1` | ✅ | AC-7 copy. |
| `scripts/install/install.sh` | ✅ | AC-7 copy. |
| `doctor/package.json` | ⏳ Wave 3 | version bump pending. |
| `doctor/package-lock.json` | n/a | no dep change; lockfile unaffected. |
| *(extra, not in IRD list)* `doctor/src/cli/service-stub.ts` | ✅ | `ServiceResult` interface (required by the structured-result change). |
| *(extra)* `doctor/src/cli/context.ts` | ✅ | `serviceStateAsync` dep field. |
| *(extra)* `doctor/tests/cli/helpers/fake-cli.ts` | ✅ | harness knob for the new dep. |
| *(extra)* `doctor/tests/cli/dispatch.test.ts` | ✅ | updated fake module to `ServiceResult` (was a build-breaker under the interface change). |
...
```
cd doctor && npm run ci   (== tsc --noEmit && vitest run)
...
`npm run audit:sql` (root) is **N/A** — the diff is confined to `doctor/`; it does not touch `src/daemon` or any Deep Lake SQL path.
...
- **Wave 3 (release):** `doctor/package.json` `0.1.9 → 0.1.10` patch bump, then commit/PR. Not a QA gap — it is the next orchestrated step.
