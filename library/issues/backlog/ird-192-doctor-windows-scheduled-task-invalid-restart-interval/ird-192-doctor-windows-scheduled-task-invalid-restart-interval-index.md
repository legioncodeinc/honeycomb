# IRD-192: Doctor Windows install-service generates invalid Scheduled Task restart interval
...
**Observed:** On Windows, `doctor install-service` writes the per-user Scheduled Task XML to
`%USERPROFILE%\.honeycomb\doctor\doctor-task.xml`, but Windows Task Scheduler rejects that
...
```text
> npm list -g --depth=0 @legioncodeinc/honeycomb @legioncodeinc/doctor
@legioncodeinc/doctor@0.1.9
@legioncodeinc/honeycomb@0.1.12

> doctor install-service
Registered the Doctor unit but a service-manager command failed (schtasks).
It will start at next login/boot; run `doctor status` to check.

> schtasks /Create /XML "%USERPROFILE%\.honeycomb\doctor\doctor-task.xml" /TN Doctor /F
ERROR: The task XML contains a value which is incorrectly formatted or out of range.
(29,24):Interval:PT5S

> schtasks /Query /TN Doctor /FO LIST /V
ERROR: The system cannot find the file specified.
```

**Expected:** `doctor install-service` on Windows should register and start a per-user
Scheduled Task without admin/UAC. The task should survive logoff/reboot, restart Doctor after a
crash, and be visible to `schtasks /Query /TN Doctor`. The installer must not tell users
"Doctor is watching" unless the service manager actually accepted the task.

---

## Impact

- Windows users can have a healthy `honeycomb` daemon and a manually running Doctor process, but
  no persistent watchdog service.
- After logoff/reboot, Doctor will not automatically start, so it cannot supervise the primary
  daemon or report incidents.
- Crash recovery is weaker than advertised because the watchdog itself is not owned by Task
  Scheduler.
- The install scripts currently suppress `doctor install-service` output and rely on exit code.
  Because the CLI returns success even when the service-manager command failed, the install path can
  produce a false-positive "Doctor is watching" message.
- `doctor status` reports `Doctor service: unknown`, which makes it hard for users and
...
The bug is the Windows Doctor service persistence layer.
...
1. On Windows, install the current published packages:
   ```powershell
   npm install -g @legioncodeinc/honeycomb@0.1.12 @legioncodeinc/doctor@0.1.9
   ```
...
4. Inspect the generated XML:
   ```powershell
   Get-Content "$env:USERPROFILE\.honeycomb\doctor\doctor-task.xml"
   ```
5. Import it directly:
   ```powershell
   schtasks /Create /XML "$env:USERPROFILE\.honeycomb\doctor\doctor-task.xml" /TN Doctor /F
   ```
...
[`doctor/src/service/templates.ts`](../../../../doctor/src/service/templates.ts) defines one
restart constant:
...
[`doctor/src/service/index.ts`](../../../../doctor/src/service/index.ts) correctly detects
that a manager command failed and returns a warning string:

```ts
return `Registered the Doctor unit but a service-manager command failed (...)`;
```

But [`doctor/src/cli/dispatch.ts`](../../../../doctor/src/cli/dispatch.ts) always returns
...
`doctor install-service` output to `/dev/null` and trusts the exit code.
...
### 4. `doctor status` service state is not wired to the real service probe

[`doctor/src/cli/index.ts`](../../../../doctor/src/cli/index.ts) currently injects:

```ts
serviceState: () => "unknown",
```

even though [`doctor/src/service/index.ts`](../../../../doctor/src/service/index.ts) exports
`serviceStatus()`. This keeps `doctor status` from becoming the simple verification command a
user needs after install.

### 5. Existing tests assert shape, not Windows import validity

[`doctor/tests/service/templates.test.ts`](../../../../doctor/tests/service/templates.test.ts)
...
2. **Make Windows XML importable by Task Scheduler.**
   - Generate XML that `schtasks /Create /XML <file> /TN Doctor /F` accepts on a non-admin
...
4. **Make installer scripts honest.**
   - Once `doctor install-service` returns non-zero on failure, ensure `install.ps1` and
     `install.sh` do not print "Doctor is watching" unless registration and start both
     succeeded.
   - On failure, print a concise warning that includes the actionable command:
     `doctor install-service`.
   - Do not block Honeycomb daemon install if Doctor registration fails, but do not report
     the watchdog as active.

5. **Wire service status into `doctor status`.**
...
6. **Add Windows-specific live validation.**
   ...
   - Add or run a Windows live smoke that creates a temporary Scheduled Task from rendered XML,
     queries it, runs it, verifies one Doctor process/status page, and deletes it.
   - The test must clean up even if assertions fail.

7. **Release as a Doctor patch.**
   - Bump `doctor/package.json` from `0.1.9` to the next patch.
   - Cut `doctor-vX.Y.Z` only after the Windows install-service proof passes.
...
| ID | Criterion |
|---|---|
| AC-1 | Given Windows and a non-admin user session, when `doctor install-service` runs, then `schtasks /Create` succeeds and `schtasks /Query /TN Doctor` finds the task. |
...
| AC-3 | Given a successful install, when `schtasks /Run /TN Doctor` runs, then exactly one Doctor process is running and its status page binds `127.0.0.1:3852`. |
| AC-4 | Given a running Doctor task, when the Doctor process exits unexpectedly, then Task Scheduler restarts it within the configured restart window. |
| AC-5 | Given a successful install, when `doctor status` runs, then it reports a non-`unknown` service state backed by the real service manager. |
| AC-6 | Given `schtasks /Create` fails for any reason, when `doctor install-service` exits, then it returns a non-zero exit code and prints a clear failure line. |
| AC-7 | Given the one-command Windows installer, when Doctor registration fails, then the installer does not print "Doctor is watching" and instead prints a non-fatal warning. |
| AC-8 | Given repeated `doctor install-service` runs, when the task already exists, then the command remains idempotent and leaves one registered task and one running watchdog process. |
| AC-9 | Given `doctor uninstall-service`, when it runs after a successful Windows install, then it stops/removes the Scheduled Task and deletes the staged XML file. |
...
- Update `doctor/tests/service/templates.test.ts` to assert the Windows restart interval is the
...
- Update `doctor/tests/service/service-module.test.ts` so a failed install command produces a
...
```powershell
npm install -g @legioncodeinc/doctor@<candidate>
doctor uninstall-service
doctor install-service
schtasks /Query /TN Doctor /FO LIST /V
schtasks /Run /TN Doctor
...
$pid = (Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -match '@legioncodeinc[\\\\/]doctor.*bundle[\\\\/]cli.js run' } |
...
```powershell
doctor uninstall-service
schtasks /Query /TN Doctor
```

### Package-level proof

- Build and pack Doctor from the repo.
...
- [`doctor/src/service/templates.ts`](../../../../doctor/src/service/templates.ts)
- [`doctor/src/service/argv.ts`](../../../../doctor/src/service/argv.ts)
- [`doctor/src/service/index.ts`](../../../../doctor/src/service/index.ts)
- [`doctor/src/cli/dispatch.ts`](../../../../doctor/src/cli/dispatch.ts)
- [`doctor/src/cli/index.ts`](../../../../doctor/src/cli/index.ts)
- [`doctor/tests/service/templates.test.ts`](../../../../doctor/tests/service/templates.test.ts)
- [`doctor/tests/service/service-module.test.ts`](../../../../doctor/tests/service/service-module.test.ts)
- [`doctor/tests/service/cli-delegation.test.ts`](../../../../doctor/tests/service/cli-delegation.test.ts)
- [`scripts/install/install.ps1`](../../../../scripts/install/install.ps1)
- [`scripts/install/install.sh`](../../../../scripts/install/install.sh)
- [`doctor/package.json`](../../../../doctor/package.json)
- [`doctor/package-lock.json`](../../../../doctor/package-lock.json)
...
- Redesigning Doctor remediation ladder behavior.
