...
### The local job queue is fleet-anchored too (the queue-scatter fix)

The vault fix moved daemon-global *config* to the fleet state root but explicitly left the memory pipeline's local job queue "per-workspace." PR #285 (v0.10.1) closed that gap: the queue is the write-side twin of the vault-scatter bug, and it is the one piece of daemon-global *durable* state that most needs to be stable.

The queue is the transactional SQLite store the pipeline runs on (`<baseDir>/.daemon/local-queue.db`), where the capture path enqueues pipeline jobs and the pipeline worker drains them across the daemon's whole lifetime (see [`../ai/memory-pipeline.md`](../ai/memory-pipeline.md) and ADR-0009, [`../architecture/adr/0009-local-queue-as-default-deeplake-is-not-a-queue.md`](../architecture/adr/0009-local-queue-as-default-deeplake-is-not-a-queue.md)). It had been opened with `resolveWorkspaceBaseDir()` = `HONEYCOMB_WORKSPACE ?? process.cwd()`, so the durable DB landed under whatever directory the daemon launched from. A restart, or a service-manager launch, from a different cwd then opens a *different, empty* queue and **orphans the pending jobs, so their memories never form**, and with `HONEYCOMB_WORKSPACE` unpinned the queue scatters across every folder the daemon happened to start in, exactly the class the vault fix closed for `.vault/`.

The fix adds `resolveLocalQueueBaseDir()` = `honeycombStateDir()` in `assemble.ts`, so the queue now opens at `<fleetRoot>/honeycomb/.daemon/local-queue.db`, cwd-independent, and a restart from any launch dir reopens the *same* queue. A companion change in `local-job-queue.ts` adds `resolveFleetRoot()` to `trustedLocalQueueRoots()` so a custom absolute `APIARY_HOME` (outside `homedir()`) is still admitted; without it the trusted-root guard would reject the home-anchored base and silently force the pipeline onto the unreliable shared DeepLake queue. The filesystem-root rejection is unchanged.
...
The same fleet-anchored `local-queue.db` now holds a second durable table. PR #287 (PRD-079a, v0.11.0) added the **`capture_outbox`**: a dedicated table for capture appends that failed against a degraded DeepLake window, so the row can be re-appended by a background drainer on recovery instead of being dropped. It reuses this file's fleet anchoring and the `local-job-queue` SQLite / trusted-root helpers (which is why it is home-anchored and survives restart), but it is a distinct table kept out of the pipeline job queue's payload guard, so the two share a DB without sharing a schema. PR #289 (PRD-079b/c, v0.12.0) hardened this table's semantics without changing its placement: rows now carry a terminal `dead` status (a permanently-failed row is retained but never re-leased), an active-backlog `maxRows` cap sheds the oldest pending rows oldest-first, and drains coalesce by scope and column shape, so the outbox is bounded rather than unbounded. The capture-side narrative is in [`../ai/session-capture.md`](../ai/session-capture.md) and the recovery mechanics in [`../storage/deeplake-recall-and-capture-findings-2026-07-10.md`](../storage/deeplake-recall-and-capture-findings-2026-07-10.md) §3.3 to §3.4.
