...
Team skills sharing is a four-step cycle that runs automatically in the background whenever Honeycomb is installed. Every step that touches storage goes through the Honeycomb daemon on port 3850; hooks and the CLI never open DeepLake directly, they ask the daemon to do it.

1. **Mine.** At session end (and periodically mid-session), a background worker reads recent session rows from DeepLake through the daemon, asks a gate model whether the activity is worth codifying, and writes a `SKILL.md` to the local `.claude/skills/` directory.
...
3. **Pull.** On every `SessionStart`, the auto-pull module asks the daemon to query the `skills` table for all users in the org and writes any newer remote skills to the local install root.
...
The skillify worker respects a scope setting persisted in `~/.honeycomb/state/skillify/config.json`:
...
On a fresh workspace, the `skills` table does not exist yet (the daemon creates it lazily on the first `INSERT`). The auto-pull uses a "trusted table list" path to detect this: it asks the daemon for the list of existing tables, and if `skills` is absent, skips the `SELECT` entirely. This prevents a relation-does-not-exist error from appearing in the daemon and DeepLake logs on every `SessionStart` for new users.
...
`backfillSymlinks` closes this gap: at the end of every pull run (except dry-runs and project-local pulls), it scans the manifest for all globally-installed entries and ensures each has a symlink in every currently-detected agent root. The cost is roughly one `lstat` syscall per (entry, detected root) pair. For a typical fleet of 50 skills and 3 agent roots that is about 150 syscalls, negligible compared to the daemon round-trip.
...
The `--force` flag is available via `honeycomb skill pull --force`. Dry-run mode (`--dry-run`) reports what would have been written without touching the filesystem.
...
Every pull writes a record to the local pull manifest so `honeycomb skill unpull` can identify and reverse pull-managed entries without relying on the `--<author>` directory naming heuristic. The manifest records the `dirName`, `name`, `author`, `projectKey`, `remoteVersion`, `install`, `installRoot`, `pulledAt`, and the list of symlinks created by fan-out.
...
flowchart TD
    ...
    insertRow["Daemon INSERT into skills table (DeepLake)"]
    nextSession["Teammate's SessionStart"]
    autoPull["autoPullSkills (5s timeout)"]
    selectSkills["Daemon SELECT from skills WHERE version > local"]
