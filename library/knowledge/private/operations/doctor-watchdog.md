...
> Category: Operations | Version: 1.4 | Date: July 2026 | Status: Active (published to npm; standalone install available, bootstrap-installer wiring landed; install surface homed in the-apiary as of PR #227)
...
- The bootstrap installers (`install.ps1` / `install.sh`, now homed in [the-apiary `scripts/install`](https://github.com/legioncodeinc/the-apiary/tree/main/scripts/install)) name the actionable recovery command, `doctor install-service`, in their failure copy; the success "watching" line was already exit-gated.
