...
> **Shipping status.** HiveDoctor is now **published** as `@legioncodeinc/hivedoctor` (npm, currently `0.1.x`) and installable on its own (`npm install -g @legioncodeinc/hivedoctor`). It versions **independently** of the root `@legioncodeinc/honeycomb` package and **never auto-updates its own package** (PRD-063 OD-6 / AC-6): esbuild inlines `hivedoctor/package.json`'s version via `define` (`__HIVEDOCTOR_VERSION__`), that one manifest is the single source of truth, and a deliberate `npm version`-style bump is the only way it moves. HiveDoctor is now developed and released directly from its own repository (`github.com/legioncodeinc/hivedoctor`): its release runs from that repo's `.github/workflows/release.yaml` (triggered by `v*` tags) and its cross-OS CI from `.github/workflows/ci.yaml`, both using the same OIDC Trusted-Publishing + provenance posture as the parent. The **bootstrap-installer wiring has now landed** (PRD-064b): the one-command installer (`scripts/install/install.sh` and `scripts/install/install.ps1`) installs HiveDoctor as a second global after the primary and registers its OS service automatically. The bootstrap is **idempotent and fail-soft** (a HiveDoctor hiccup never fails the Honeycomb install) and honors a single install-time opt-out: `--no-hivedoctor` (bash), `-NoHiveDoctor` (PowerShell), or `HONEYCOMB_NO_HIVEDOCTOR=1` (env) -- the installer guard mirrors `hivedoctor/src/service/install-guard.ts` (`shouldBootstrapHiveDoctor`) and must stay in sync with it. The root README documents this customer-facing behavior as of PR #168. The standalone install (`npm install -g @legioncodeinc/hivedoctor`) remains available for opt-out or manual setups.
