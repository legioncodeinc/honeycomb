...
> Category: Operations | Version: 1.3 | Date: July 2026 | Status: Active
...
The write stage adds one diagnostic the driver-heartbeat events do not carry. When the controlled-writes dedup SELECT fails for a genuine reason (not a heal-able missing table or column), the stage emits `controlled_write.dedup_probe_failed { classification, kind, status, transient, reason }` and throws an enriched error that lands in the queue's `last_error_class` (BUG-04, PR #293, v0.12.2). Before this, a memory dropped here against an opaque `query_error` that hid the real DeepLake error text and HTTP status, so an operator could not tell a 5xx backend flap from a 402 balance exhaustion from a permission fault. The event and the enriched error now name the failure class and status. The SHA-256 `content_hash` the probe SQL interpolates is stripped before it reaches either surface, so this diagnostic inherits the no-secret invariant below. The full mechanics, including the correctness-preserving safety throw and the durability follow-up (BUG-04b) that PRD-080 later closed, are in [`../ai/memory-pipeline.md`](../ai/memory-pipeline.md).

### Controlled-write durability (the memory outbox backlog)

BUG-04a made the drop *diagnosable*; it did not make the memory *survive* a window that outlasts its 5 job attempts. PRD-080 (PR #294, v0.13.0) closes that gap the same way PRD-079a closed it for capture: a controlled-write that fails against a **transient** DeepLake window is no longer dropped after 5 attempts but persisted to a durable `memory_outbox` (sibling to `capture_outbox` in the home-anchored `local-queue.db`) and re-committed by a background drainer when the backend recovers (see [`../ai/memory-pipeline.md`](../ai/memory-pipeline.md)). Like capture, that turns a silent loss into a *visible backlog* that needs its own signal.

`/health` therefore gains a `memoryOutbox { pending, retrying, deadLettered }` block, built by the same pure `health.ts` contract module and present only when the composition root wires the outbox: `pending` is the count of deferred controlled-writes waiting to re-commit, `retrying` the count currently in backoff, and `deadLettered` the terminal count of rows that exhausted `maxAttempts` (default 10) or aged past `maxAgeMs` (default 24h). A non-zero-and-climbing `pending` under an otherwise-`ok` reachability bit is the tell that memory *formation* is timing out on backend writes even though a `SELECT 1` probe passes, distinct from and read the same way as the capture outbox's backlog above. The drainer emits the secret-free `memory.outbox.{enqueued,drained,retry,dead_lettered,shed}` events (counts, durations, attempt/age numbers, and action-class only, never memory content, `content_hash`, org, or workspace), so the enqueue-then-commit lifecycle is traceable exactly like the capture and pipeline lifecycle events. Because the drainer re-commits on a recovered-window signal (the next successful pipeline write and the `deeplake.woke` wake) rather than only on its interval, `pending` collapses toward `0` the moment DeepLake recovers, and a climbing `deadLettered` is the "some distilled memories are permanently unwritable" signal.

For the memories that dropped *before* PRD-080 shipped, the operator has a one-time recovery: `honeycomb memory redrive` (POSTing to the protected `POST /api/diagnostics/memory-redrive`) reads the terminal `memory_controlled_write` jobs still in `local-queue.db`, re-enqueues their resolved writes into `memory_outbox`, and reports the re-enqueued and skipped counts. The replay is idempotent by `content_hash`, so a memory a prior attempt actually landed is re-`deduped` rather than duplicated. This recovered the ~101 already-dropped memories from BUG-04's live window.
