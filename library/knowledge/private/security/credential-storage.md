...
Documents where Honeycomb stores device-flow credentials on disk, the file-system permissions enforced on every write, the shape of the credentials object, and the three file-IO helpers that own all access to the credentials file.
...
| Name | Value | Notes |
|---|---|---|
| `configDir()` | `~/.honeycomb` | Parent directory. Resolved via `homedir()` at call time. |
| `credsPath()` | `~/.honeycomb/credentials.json` | Full path to the credentials file. |
...
|---|---|---|
| `~/.honeycomb/` (directory) | `0700` (`rwx------`) | Owning user only |
| `~/.honeycomb/credentials.json` | `0600` (`rw-------`) | Owning user only |
...
|---|---|---|---|
| `token` | `string` | yes | Long-lived org-bound JWT (365-day expiry). Bearer token for all daemon and API calls. |
...
Reads and JSON-parses `~/.honeycomb/credentials.json`. Returns `null` for any failure: missing file (`ENOENT`), permission denied, or malformed JSON. Callers treat `null` as "not logged in" and prompt the user to run `honeycomb login`. The anti-pattern of `existsSync` followed by `readFileSync` is deliberately avoided; it introduces a TOCTOU race and extra branches with no safety benefit.
...
The tradeoff is that `~/.honeycomb/credentials.json` is readable by any process running as the same OS user. The mitigations are:
