...
> Category: Ai | Version: 1.2 | Date: July 2026 | Status: Active (math merged 2026-06-26 via PR #125; the five production seams and four maintenance workers wired 2026-07-05 via PR #258)
> **Ship state (July 2026).** PRD-058's lifecycle math landed on `main` on 2026-06-26 (PR #125, feature commit `6fc192d`), but the math shipped **inert**: five production seams in `assemble.ts` were never constructed and four maintenance workers were missing, so every `memories` row read `access_count=0`, `last_reinforced_at=epoch`, `ref_status=NULL`, and `importance=0.5`. The ranker had signal-carrying seams but nothing feeding them. PR #258 (2026-07-05) closed that gap: it wired the five seams and shipped the four missing maintenance workers, so the lifecycle terms now bite on the live recall path instead of reading their neutral defaults. The 151-test math layer from PR #125 is unchanged; #258 is the assembly-and-workers layer on top of it. The three credential-gated acceptance criteria (IDX-1 numeric `eval:recall` half-life verdict, IDX-7 `useful-context@k` uplift, IDX-8 live dogfood) still need `HONEYCOMB_DEEPLAKE_TOKEN` + the embed daemon and have not been run against the golden set; that credential boundary, not missing code, is what keeps PRD-058 in `library/requirements/in-work/` rather than `completed/`.
...
| Term | Math module(s) | Live recall integration | Ships (post-#258) |
|---|---|---|---|
| `A(m,t)` activation | `recall.ts` `recencyActivation` (Stage 1), `activation.ts` `actrActivation` (Stage 2) | the recency-activation stage, last among score adjustments | Stage 1 ACTIVE; Stage 2 seam now wired (`activationSource`) so ACT-R replaces the Stage-1 fallback |
| `C(m)` calibrated confidence | `calibration.ts` (isotonic fit, ECE/Brier), `calibration-store.ts` | emitted as `calibratedConfidence`, exponent `c` | calibration stage now runs via the `calibrationModel` seam; cold-start blob is IDENTITY, so `C = f` until outcomes accumulate |
| `σ(m,t)` staleness | `maintenance/stale-ref-diagnostic.ts`, `maintenance/reference-extract.ts` | folded into the recency stage as `(1 − σ)^s` | detection ACTIVE; the `stalenessSource` seam now feeds the ranker so the `σ` multiplier runs (demotion still posture-gated) |
| `κ(m,t)` conflict gate | `conflict-detect.ts`, `conflict-resolve.ts`, `memories/keep-both-memo.ts` | recall-time suppression of the open-conflict loser | ACTIVE (detect + suppress wired); `KeepBothMemoStore` now backs in-process keep-both |
...
- **Activation `A^a` - LIVE.** Stage 1 recency is on with class-aware half-lives (180 / 45 / 10 days) and `a = 1.0`. It reorders equally-relevant hits by age; it is no longer the 100-year no-op, and recency genuinely bites once memories span weeks. Stage 2 (ACT-R `A_actr`) was built and unit-tested behind the `activationSource` seam and, pre-#258, ran the byte-for-byte Stage-1 fallback because no source was injected. PR #258 now injects `activationSource` in `assemble.ts`, so ACT-R replaces the Stage-1 fallback and the reinforcement loop feeds live recall.
...
## PR #258: wiring the inert lifecycle

Between PR #125 and PR #258 the lifecycle was a math layer with nothing feeding it. The 151 unit tests passed, but at runtime every `memories` row read its neutral default (`access_count=0`, `last_reinforced_at=epoch`, `ref_status=NULL`, `importance=0.5`), because the seams the recall engine reads from were never constructed in `assemble.ts` and the maintenance workers that stamp the rows did not exist. PR #258 (2026-07-05) filled both halves.

**Five seams wired in `assemble.ts`:**

- `recordRecallAccess`: every recall hit now bumps `access_count` and advances `last_reinforced_at`, so the reinforcement series stops being empty and `t_ref = max(created_at, last_reinforced_at)` starts moving.
- `activationSource`: the ACT-R Stage-2 activation is now injected, replacing the Stage-1 recency fallback that recall ran when the seam was absent.
- `calibrationModel`: the calibration stage now runs. Its cold-start blob is the IDENTITY curve, so `C = f` until enough resolved outcomes accumulate and ECE clears its gate. The computation happens; it simply does not perturb ranking yet.
- `stalenessSource`: the `σ` staleness multiplier now runs inside the ranker's recency-activation step rather than degrading to neutral for want of a source.
- `lifecycleConfig`: `resolveLifecycleConfigLayered()` is now read at boot, and the resolved `a` / `c` / `s` / posture values are threaded into the recall and conflict dependencies instead of falling back to hard-coded defaults.

**Four maintenance workers shipped** (`src/daemon/runtime/maintenance/`): `calibrate-api.ts` (refits the isotonic curve), `compact-access-log-api.ts` (compacts raw `memory_access` events into the `access_count` + `last_reinforced_at` denormalized cache), `reverify-api.ts` (drives spaced re-verification of stale references), and `lifecycle-tick.ts` (the periodic driver that schedules the above). Alongside them, `KeepBothMemoStore` (`memories/keep-both-memo.ts`) backs the in-process keep-both conflict outcome, and `memories/{api,calibration-store}.ts` carry the read/write surface these workers use.

The safety posture is unchanged by #258: each seam still has a neutral fail-soft default, so a missing or throwing source degrades that term to its identity factor rather than breaking recall (see "The fail-soft guarantee"). What changed is that on a normal boot the seams are now present, so the terms compute against real data instead of silently reading their epoch/zero/NULL defaults.
...
This whole loop was built, unit-tested, and CI-green from PR #125, but until PR #258 it was **dormant in the live recall deps**: `recallMemories` ran the Stage-1 recency path because nothing injected an `activationSource`, and nothing bumped `access_count` / `last_reinforced_at` on a recall hit. PR #258 wired both ends in `assemble.ts`, the `recordRecallAccess` seam on the write side and the `activationSource` seam on the read side, so the reinforcement loop now closes on the production recall handler.
