...
BUG-04a (PR #293) was diagnostic-only. It made the next degraded window diagnosable and preserved correctness, but it did not by itself make a memory survive a window that outlasts its 5 job attempts. That durability gap was BUG-04b, and PRD-080 (PR #294, v0.13.0) closes it: the write-side twin of the capture path's `capture_outbox` (see [`session-capture.md`](session-capture.md) and [`../storage/deeplake-recall-and-capture-findings-2026-07-10.md`](../storage/deeplake-recall-and-capture-findings-2026-07-10.md) §3.3). At the two controlled-write throw points the stage now gates on `isTransientResult` before it throws: a **transient** failure (5xx, 429, timeout, dropped connection, eventual-consistency flap) enqueues the resolved write, the `{ action, built row, scope }` tuple, into a durable `memory_outbox` and returns a `deferred` action so the `memory_controlled_write` job acks and never burns its 5 attempts; a **genuine** non-transient failure (permission, syntax, a hard balance fault) still throws exactly as before, so the never-an-unguarded-duplicate-insert invariant is preserved unchanged. A background drainer re-executes only the commit on the dedicated write client (`Semaphore(3)`), not extraction or decision, which already ran, with bounded exponential backoff: an ADD replay re-runs the dedup probe, so a memory a prior attempt already landed comes back `deduped` via `content_hash` (idempotent by construction, no duplicate row) and UPDATE/DELETE replays via the version-bumped write; an OK/deduped row is deleted, a transient re-fail bumps `attempts` and pushes `next_attempt_at`. `commitControlledWrite` / `commitControlledWriteMany` are factored out and single-sourced by both the live stage and the drainer, so first-write and replay share one dedup-idempotent path. The outbox rides the same home-anchored `local-queue.db` as `capture_outbox` (`honeycombStateDir()`, PR #285), so a queued write survives a daemon restart and drains on the next boot, and the whole subsystem is fail-soft: an enqueue or drain fault degrades to the pre-080 throw rather than breaking the stage. Mirroring PRD-079b/c, PRD-080 also adds a terminal `dead` status after `maxAttempts` (default 10) or `maxAgeMs` (default 24h), a recovery-triggered drain kicked on the next successful pipeline write and on the `deeplake.woke` wake, an oldest-first `maxRows` shed (default 10k, always counted via `memory.outbox.shed`, never silent), and coalesced multi-row version-bumped appends grouped by scope and column signature. Drain-recovered commits feed the memory-formation tracker so `committedSinceBoot` climbs *through* a degraded window, which is BUG-04's Verify signal. Finally, `honeycomb memory redrive` (a CLI verb plus a `protect:true` `POST /api/diagnostics/memory-redrive` route sharing one `runMemoryRedrive` seam) reads the terminal `memory_controlled_write` jobs still in the queue and re-enqueues their resolved writes into the outbox, recovering the ~101 memories that dropped before PRD-080 shipped; the re-drive is idempotent by the same `content_hash` dedup. With durability closed on top of BUG-04a's diagnostics, BUG-04 flips green. The mechanics live in `src/daemon/runtime/pipeline/{memory-outbox,memory-redrive,memory-redrive-api,controlled-writes}.ts` and `src/commands/memory.ts`; the operator surface (`/health memoryOutbox { pending, retrying, deadLettered }`, the secret-free `memory.outbox.*` events) is in [`../operations/observability-and-degradation.md`](../operations/observability-and-degradation.md).
