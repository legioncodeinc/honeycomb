...
> **PRD:** `prd-067-doctor-boot-grace-release-blocker`
...
1. Add a `DOCTOR_STATUS_PAGE_PORT` env/config field so tests and operators can choose port `0` or another loopback port.
2. Add a process-level keepalive handle for `run` so Doctor remains alive even when the status page cannot bind.
3. Add single-instance detection or a lock file for Doctor itself, so duplicate runs exit with an explicit diagnostic instead of a quiet early exit.
4. Treat status-page bind failure as degraded-but-running, and expose it in `doctor status` / logs.
