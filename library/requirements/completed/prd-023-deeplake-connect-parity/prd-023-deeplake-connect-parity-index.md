...
- Hivemind connect: `<hivemind-source>\src\commands\auth.ts` (device flow + token mint + `/me` + `/workspaces`), `src\commands\auth-creds.ts` (creds file shape/path), `src\config.ts` (env + creds resolution), `src\deeplake-api.ts` (transport — already mirrored).
