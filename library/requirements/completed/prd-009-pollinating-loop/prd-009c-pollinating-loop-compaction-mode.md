...
- Clean up a graph that grew messy before pollinating was enabled, in one full-graph pass, so accumulated duplicates and junk are reconciled at once.
- Let an operator force a full pass on demand via `honeycomb pollinate trigger --compact`, regardless of the token counter state.
...
- Selecting the model; the pollinating workload resolves through the router (PRD-010).

## User stories

- As an operator with a graph that grew messy before pollinating was enabled, I want a single full-graph pass so that accumulated duplicates and junk get cleaned up at once.
...
- FR-1: When `backfillOnFirstRun: true` and no prior pollinating pass exists for a scope, the first pollinating run MUST enter compaction mode instead of an incremental pass.
- FR-2: `honeycomb pollinate trigger --compact` MUST queue a full-graph compaction pass for the target scope regardless of `tokens_since_last_pass`.
...
- FR-4: The runner MUST sample recent summaries rather than loading all of them, bounding input to `memory.pollinating.maxInputTokens`.
...
- FR-8: On completion, compaction MUST update `pollinating_state.last_pass_at` and clear `pending_job_id`, so subsequent passes return to incremental mode.

## Acceptance criteria

| ID | Criterion |
|---|---|
| AC-1 | Given `backfillOnFirstRun: true` and no prior pass, when pollinating first runs, then it enters compaction mode and walks the full graph instead of the incremental payload. |
| AC-2 | Given `honeycomb pollinate trigger --compact`, when it runs, then a full-graph compaction pass is queued regardless of the token counter state. |
...
- `honeycomb pollinate trigger --compact` is exposed by the `honeycomb pollinate` CLI verb (parent index, API changes) and enqueues into the same `memory_jobs` queue PRD-009b consumes.

## Dependencies

- PRD-009b session runner (lifecycle, mutation apply, model resolution).
- PRD-009a `pollinating_state` row (for `last_pass_at` and pending-job bookkeeping).
- `honeycomb pollinate` CLI verb and daemon endpoint.
