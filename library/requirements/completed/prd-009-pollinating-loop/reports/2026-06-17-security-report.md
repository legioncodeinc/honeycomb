# Security Audit — PRD-009 Pollinating Loop

- **Date:** 2026-06-17
- **Auditor:** security-worker-bee
- **Branch:** prd-009-pollinating-loop
- **Scope:** PRD-009 pollinating loop (penultimate /the-smoker step, run BEFORE quality)
...
The PRD-009 pollinating loop reasons over the knowledge graph with a stronger model and
...
2. **Scope isolation** — the org/workspace partition is credential-bound (`QueryScope`,
   never caller/model-named); every pollinating read carries the `agent_id` conjunct. A
...
scan `src/cli/pollinate.ts`. This is acceptable: `pollinate.ts` is a thin client that imports
...
|---|----------|-------|-----------|--------|
| 1 | Medium | Counter wedge (availability) | `src/daemon/runtime/pollinating/trigger.ts:208` | Documented — design-acknowledged; verify at daemon assembly |
| 2 | Low | Counter integer overflow (integrity) | `src/daemon/runtime/pollinating/trigger.ts:313` | Documented |
| 3 | Low | `audit:sql` does not scan the CLI | `scripts/audit-sql-safety.mjs:48` / `src/cli/pollinate.ts` | Documented — no SQL sink in CLI |
...
Every interpolation in trigger / incremental / compaction / pollinating-state / control-plane
is guarded:
...
- `pollinating-state.ts` — defines columns only; every `NOT NULL` carries a `DEFAULT` so
...
CLI: `src/cli/pollinate.ts` builds no SQL (thin client, injected seams). Acceptable that
...
- The **agent_id** inner ring is on EVERY pollinating read: counter reads
...
- **No lost-write on the reset race** — verified live: 180 − 100 → 80 carried forward,
  not hard-zeroed (`pollinating-counter-live.itest.ts`, both tests pass).
...
unescaped (control-plane writes them via `val.text` → `eLiteral`). The `POLLINATING.md`
task prompt is pollinating-only by construction (reached only from
...
`src/daemon/runtime/pollinating/trigger.ts:208`
...
and the scope wedges — no further pollinating passes for that (org, workspace, agent_id).
...
`src/daemon/runtime/pollinating/trigger.ts:313`

```ts
const next = state.tokensSinceLastPass + delta;
```

`incrementPollinatingCounter` floors `delta` at 0 and truncates it, but applies no upper
...
`scripts/audit-sql-safety.mjs:48` (`SCAN_DIR ?? "src/daemon"`) / `src/cli/pollinate.ts`

The SQL-safety gate scans `src/daemon` only; the CLI is not covered. Today this is safe
because `src/cli/pollinate.ts` builds no SQL and holds no storage handle (verified). But a
...
| Token / credential in logs | None detected (no logging of secrets in any pollinating file) |
...
| Live pollinating counter | `vitest run … pollinating-counter-live.itest.ts` | exit 0 — 2/2 passed |
