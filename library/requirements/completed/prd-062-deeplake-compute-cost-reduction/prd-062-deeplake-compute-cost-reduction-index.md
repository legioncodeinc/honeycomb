...
| [`prd-062e-…-idle-hibernation`](./prd-062e-deeplake-compute-cost-reduction-idle-hibernation.md) | **The follow-on to 062b: idle to zero.** 062b backed the idle poll cadence off to a ~30s ceiling, but a query every 30s still resets Activeloop's compute idle-timer, so compute never scales to zero. 062e makes the adaptive loop **stop re-arming entirely** after a configurable idle window (zero DeepLake polls), with a `wake()` seam fired by the enqueue chokepoint; it also hibernates the queue reaper and converts the still-flat summary + skillify workers onto the same adaptive loop. Env-flagged, default-on, rollback restores 062b's steady cadence. | Draft |
