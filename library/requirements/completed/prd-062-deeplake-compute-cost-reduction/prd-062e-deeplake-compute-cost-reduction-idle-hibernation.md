...
- **Instant, correct wake.** Any inbound request resumes every paused handle through the controller's `touch()`; a woken worker's adaptive loop snaps back to the fast floor on its first leased job, so active-session pickup latency is unchanged.
...
- **The wake signal is inbound HTTP, registered once.** A root middleware (`daemon.app.use("*", ...)`) calls `touch()` before any route group, so every capture/recall/CLI request resumes the fleet without instrumenting each handler. Background worker queries are not inbound requests, so they never spuriously keep the daemon awake - only real agent activity does.
...
| AC-62e.6 (parity) | With `HONEYCOMB_DEEPLAKE_HIBERNATE_ENABLED=false` (or `0`) `start()` is a no-op and nothing is ever paused (062b's steady ceiling cadence); the idle window clamps up to its floor and ignores a non-numeric value. |
...
- `src/daemon/runtime/assemble.ts` - wire the controller at the root: the `touch()` middleware, the arm helpers for the health probe / graph build / maintenance tick, the pausable set (workers + maintenance tick + health probe + graph build), and start/stop across the daemon lifecycle.
