# PRD-026 — Pollinating loop enablement + live validation (turn it ON, prove it consolidates)

> Status: completed · Owner: `/the-smoker` · Type: M (feature)
> Goal: turn the PRD-009 Pollinating consolidation loop ON (trivially enableable, with run-away guards) and
...
> **Reconciliation (2026-06-22):** this PRD was implemented in an earlier wave (the `HONEYCOMB_POLLINATING_ENABLED`
> enablement gate + the `pollinating-counter-live` / `pollinating-consolidation-live` itests) but never got a QA
...
`POST /api/diagnostics/pollinate` today returns `{triggered:false, status:"skipped", reason:"disabled"}`
(`src/daemon/runtime/pollinating/api.ts`). The whole consolidation loop is BUILT — the token-budget trigger
...
— but `resolvePollinatingConfig` (`config.ts`) defaults `memory.pollinating.enabled` to **false** (a premium tier,
...
- **Enablement.** Make `memory.pollinating.enabled` trivially turn-on-able — flip the default (D-1) and/or a
  first-class enable path — without touching the trigger/runner internals. Re-resolve config so the live
  `/api/diagnostics/pollinate` stops returning `reason:"disabled"` once enabled.
...
- **What a pass may mutate.** Keep every mutation on the 008c risk-routed apply path (`runner.ts` →
  `submitProposal`): the seven-op vocabulary (`contracts.ts` `POLLINATING_MUTATION_KINDS`) where destructive
...
- **The behavioral proof.** A gated live itest that seeds a workspace with KNOWN duplicates + stale claims +
  junk, runs a real Pollinating pass against live DeepLake (real `pollinating` ModelClient call), and asserts the
...
- **D-1 — Enablement posture: trivially-enableable, default stays conservative until proven.** Keep
  `memory.pollinating.enabled` false-safe in `config.ts` as the SHIP default (a missing flag is OFF — premium
  tier, no surprise model spend), but make ON a one-knob flip: `HONEYCOMB_POLLINATING_ENABLED=true` (already the
...
- **D-2 — Cadence is the existing token-budget trigger, unchanged.** A pass fires when
  `pollinating_state.tokens_since_last_pass` crosses `tokenThreshold` (default 100k) on a maintenance tick, OR
  on the explicit `/api/diagnostics/pollinate` / `honeycomb pollinate trigger` path. No clock-based cadence is added;
...
- **D-5 — The model workload is the `pollinating` ModelClient seam, resolved by the router.** A live pass calls
  the real model through the PRD-010 `pollinating` workload (the seam PRD-009b wired); this PRD does not select
...
- **AC-1 — Enable flips the live trigger.** With `memory.pollinating.enabled` true,
  `POST /api/diagnostics/pollinate` no longer returns `reason:"disabled"`: it returns `{triggered:true,
...
- **AC-2 — Cadence + single-pending guard hold live.** Summary-write increments accumulate
  `tokens_since_last_pass`; crossing `tokenThreshold` enqueues exactly ONE `pollinating` job and resets by
...
- **AC-3 — A real pass consolidates a seeded set (the behavioral bar).** Gated live itest: seed a workspace
  ...
  entity. Run ONE real Pollinating pass against live DeepLake (real `pollinating` model call). After the pass,
...
- **AC-6 — Safety + gates green.** Destructive mutations land in pending review (never blind-applied); the
  pollinate-trigger ack carries no token/secret (D-4 of PRD-024 unchanged); `npm run ci`, `build`, `audit:sql`,
...
- **PRD-009 (Pollinating Loop)** — the trigger, runner, strategies, contracts, and config this PRD enables. This
...
- **PRD-010 (Model & Provider Router)** — the `pollinating` workload seam the live pass calls (D-5).
- **Storage write model** — `appendVersionBumped` (claim history, append-only) + the `pollinating_state` counter
  in `src/daemon/storage/`; poll-convergent reads are mandatory on every live read-back.
- **PRD-030 (Memory compaction)** likely RIDES this PRD's enabled runner (compaction-as-pollinating-pass option),
  so 030 depends on 026.

## Reference

- Loop to enable: `src/daemon/runtime/pollinating/{trigger.ts,runner.ts,config.ts,contracts.ts,incremental.ts,compaction.ts}`.
- Live trigger endpoint: `src/daemon/runtime/pollinating/api.ts` (`POST /api/diagnostics/pollinate`).
- Apply path: the PRD-008 ontology control plane (`submitProposal`, risk router, pending review).
- Storage write model: `src/daemon/storage/writes.ts` (`appendVersionBumped`), the `pollinating_state` catalog table.
- Parent feature: `library/requirements/in-work/prd-009-pollinating-loop/`.
