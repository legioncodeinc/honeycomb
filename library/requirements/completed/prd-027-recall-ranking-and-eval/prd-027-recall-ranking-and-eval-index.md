...
> **Reconciliation (2026-06-22):** this PRD had no QA report and a stale `backlog` label, but the work
> shipped in commit `405efcf` (#48) — RRF fusion (`RRF_K`, `fuseHits`, arm-class weights) in
> `src/daemon/runtime/memories/recall.ts` plus the gated `npm run eval:recall` harness. Verified live on
> `main` (the arm-order/placeholder-score defect is absent). Complete. See `library/ledger/EXECUTION_LEDGER.md`.
