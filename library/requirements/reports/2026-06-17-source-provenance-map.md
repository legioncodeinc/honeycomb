...
|---|---|---|
| our memory engine | `<memory-engine-source>\` | Full source. `platform/core` (78 migrations, ingest, fts), `platform/daemon` (HTTP, pipeline, workers, auth, MCP), `platform/daemon-rs` (Rust parity), `platform/native`, `surfaces/*`, `integrations/*`, `libs/*`, `plugins/core/secrets`. |
...
Foundation from hivemind, intelligence from our memory engine, with as little of our memory engine's code carried as possible.
...
- Reimplement our memory engine DeepLake-native, using our memory engine's source as the algorithm reference, not as code to port: the pipeline, hybrid retrieval and ranking, the knowledge-graph ontology and supersession, the pollinating loop, the model and provider router, and agent scoping.
- The daemon is mostly net-new. hivemind is hooks-direct with no daemon; our memory engine's daemon is SQLite-coupled. Build a daemon shell that wraps hivemind's `deeplake-api` as the only client and hosts the reimplemented engine and workers, using our memory engine's daemon structure as a reference.
...
| PRD | Module | hivemind source | our memory engine source | Classification |
|---|---|---|---|---|
...
| 003 | core-data-model | `deeplake-schema.ts` column-def pattern | `platform/core/src/migrations/*` (schema shape) | net-new defs, reference our memory engine |
| 004 | daemon-runtime | none (hooks-direct) | `platform/daemon/src` routes, job queue, watcher | net-new + reference our memory engine |
| 005 | capture-intake | `src/hooks/*` (capture, session-*, pre-tool-use), `embeddings/client.ts` | `ai/session-capture` design | adapt hivemind |
| 006 | memory-pipeline | none | `platform/daemon/src/pipeline/*` (extraction, decision, worker, retention) | reimplement, reference our memory engine |
| 007 | retrieval | `embeddings/*` vector path, `graph/vfs-handler` | `memory-search.ts`, ranking, dampening | reimplement (reference our memory engine) + reuse hivemind vectors |
| 008 | knowledge-graph-ontology | none (codebase graph is different) | `knowledge-graph.ts`, `ontology-*.ts`, `pipeline/supersession.ts` | reimplement, reference our memory engine |
| 009 | pollinating-loop | none | `pipeline/pollinating*.ts` | reimplement, reference our memory engine |
| 010 | model-provider-router | host-CLI gate pattern in `hooks/*/wiki-worker` | `inference-router` and model catalog | reimplement, reference our memory engine |
...
| 012 | secrets | none | `plugins/core/secrets`, daemon secrets routes | reimplement, reference our memory engine |
| 013 | sources-and-documents | ingest helpers | `ingest/*` parsers (reuse), source providers + document worker | reuse our memory engine parsers + reimplement lifecycle |
...
| 017 | wiki-summaries | `hooks/*/wiki-worker.ts`, `spawn-wiki-worker` | `pipeline/summary-worker.ts`, synthesis | adapt hivemind + reference our memory engine |
...
Reimplement from our memory engine as reference (the memory intelligence): 006, 007, 008, 009, 010, 012, and the source lifecycle half of 013. These are our memory engine's strength and the reason for the merge, but they ride on SQLite transactions and must be rewritten on the storage adapter, not ported.

Net-new shell: 004 (the daemon) and the unified monorepo in 001.

Pure-logic reuse from our memory engine (low coupling, safe to lift): the `platform/core/src/ingest/*` parsers (chat, discord, slack, markdown, pdf, code, git) used by 013.

## Risks and notes

The transaction rewrite is the single biggest risk and lands in 006, 007, 008. Treat our memory engine's `withWriteTx` closures as design notes for what must be atomic, then express that atomicity with the DeepLake patterns (append-only version bumps, SELECT-before-INSERT, content-hash dedup) from 002.
