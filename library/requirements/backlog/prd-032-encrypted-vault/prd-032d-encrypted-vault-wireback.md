# PRD-032d — Vault wire-back: assembly reads provider/model + dreaming from the vault
...
The wire-back that makes the vault the live source of truth for inference selection and the dreaming toggle.
At assembly, the daemon READS the active provider/model and the `dreaming.enabled` flag from the vault
`setting` class (PRD-032 D-5, option B), falling back to the committed `agent.yaml` and the
`HONEYCOMB_DREAMING_ENABLED` env var only when no vault setting exists. The vault does NOT generate or
...
- Assembly resolves `dreaming.enabled` from the vault, with `HONEYCOMB_DREAMING_ENABLED` as the fallback.
...
- Re-architecting the router or the dreaming loop (PRD-010 / PRD-009 / PRD-026 own those).
...
- FR-2: The inference CREDENTIAL keeps resolving through the vault `secret` class via the existing
  ...
  never inlined).
- FR-3: The `dreaming.enabled` flag resolves from the vault `setting` class when present; absent it, it falls
  back to `HONEYCOMB_DREAMING_ENABLED` (`src/daemon/runtime/dreaming/config.ts`), preserving PRD-026 behavior.
...
| d-AC-2 | Given a vault `setting` `dreaming.enabled=true`, when assembled, then `POST /api/diagnostics/dream` no longer returns `reason:"disabled"` (PRD-026 behavior) WITHOUT `HONEYCOMB_DREAMING_ENABLED` being set. |
| d-AC-3 | Given NO vault settings, when assembled, then provider/model falls back to `agent.yaml` and dreaming falls back to `HONEYCOMB_DREAMING_ENABLED` — existing installs are unchanged. |
...
- Thread a vault-settings read into the assembly inference path (`assemble.ts` → `buildInferenceModelClient`)
  ...
  the `dreaming.enabled` vault read in front of the env read in `dreaming/config.ts` (vault → env → false-safe).
...
- PRD-026 dreaming-enabled semantics (`src/daemon/runtime/dreaming/config.ts`, `api.ts`).
