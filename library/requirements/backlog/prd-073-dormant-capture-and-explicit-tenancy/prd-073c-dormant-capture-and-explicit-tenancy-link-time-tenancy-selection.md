...
- `GET /setup/tenancy` -> `{ pending: boolean, selected: boolean, authenticated: boolean, org: {id, name} | null, workspace: {id, name} | null, autoSelected?: {orgId, workspaceId} }`. `pending` is true only during an unconsumed link window; `selected` is true ONLY after an explicit select has persisted (a login-time guess never reads as selected); when neither, it reports the persisted credential tenancy so an already-linked machine renders honestly.
