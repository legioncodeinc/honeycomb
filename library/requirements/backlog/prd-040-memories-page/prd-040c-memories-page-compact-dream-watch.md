# PRD-040c: Compact + dream + watch the memory lifecycle
...
- **DREAM** — trigger the consolidation loop (PRD-009 / PRD-026) via the existing
  `POST /api/diagnostics/dream` (`src/daemon/runtime/dreaming/api.ts`), reflected HONESTLY: the 202 ack distinguishes
  ...
  current home-page "Dream now" button already implements (`app.tsx` `dream`), never a fake forever-spinner.
- **WATCH** — a live-tail of memory activity. There is no memory-specific event stream today; the live feed is the
  ...
  the dream/compact triggers), for a zero-new-endpoint start; a dedicated SSE memory-events stream is captured as a
  deferred enhancement (OQ).

Both Compact and Dream invoke the REAL pipelines through endpoints the daemon already serves — this sub-PRD wires
the UI + the typed wire methods (the wire client has `dream` today but no `compact`), it adds no new pipeline logic.
...
- A DREAM control that POSTs `/api/diagnostics/dream` and reflects the ack honestly — enqueued vs already-running vs
...
- New compaction or dreaming PIPELINE logic — both are triggered via existing endpoints; this sub-PRD is the UI +
...
- Scheduling / cron for compaction or dreaming — these are manual, user-triggered controls here; the daemon's own
  background cadence (the dreaming loop, the maintenance schedule) is unchanged and out of scope.
- Team/hybrid-mode triggers — LOCAL-MODE single-tenant only; the triggers inherit the diagnostics group's local-open
  posture exactly as the home-page Dream button does.

## User Stories

- As a developer, I want to manually kick a dreaming consolidation pass and SEE honestly whether it was queued,
...
- As a developer, I want to WATCH memory activity live — writes, recalls, dream/compact passes — as it happens, so I
  can observe the system working.

## Implementation Notes

- **Wire methods (`src/dashboard/web/wire.ts`):**
  - `dream()` — ALREADY EXISTS (returns the honest `DreamAck` `{ triggered, status, reason }`). Reused as-is.
...
- **Dream control (honest ack, reused):** lift the EXACT logic from `app.tsx`'s `dream` callback — POST, then branch
...
- **Watch mode (poll-filter start):** a toggle that starts the `app.tsx` log-poll recipe (mount-fetch +
  ...
  `/api/diagnostics/dream`, `/api/diagnostics/compact`) via the existing `formatLogLine`. Stopping the watch clears
  the interval. The log records carry no secret/token (the logger redacts by construction); the filter introduces
  none.
- **Eventual consistency:** after a compact or dream, the corpus/log may not reflect the result on the very next
...
- **No new daemon surface (default):** compact + dream + the log poll all hit existing endpoints. The only candidate
...
- [ ] **AC-2 — Dream invokes the real loop + honest ack.** The Dream control POSTs `/api/diagnostics/dream` and
  reflects the ack honestly — enqueued vs already-running vs skipped (with the reason) — reusing the no-fake-spinner
  pattern; a `triggered:false` ack shows "skipped", never a forever-dreaming state. Tested across the three ack shapes.
...
- [ ] **AC-4 — Acks/summaries are state-only.** No token/secret/header renders in any compact summary, dream ack, or
...
- **OQ-3 (parent OQ-4) — Dream control duplication with the shell.** PRD-037 D-5 keeps a global "Dream now" in the
  shell chrome. If the Memories page ALSO surfaces a Dream control, there are two. Proposed: keep both (the global is
