...
- [x] **Dual pid stamping during the window** (migration mechanics step 5.5): RESOLVED 2026-07-04 (QA Warning 2 reconciliation): dual-stamp is WINDOW-ONLY, applied only when the legacy `~/.honeycomb` directory ALREADY EXISTS (an upgrade mid-window); a fresh install never creates the legacy dir. This reconciles index AC-1 (no `~/.honeycomb` on a fresh install) with mechanics step 5.5 (old doctors and operator muscle memory keep working across upgrades). Both branches are pinned by tests in `tests/daemon/runtime/lock-continuity.test.ts`.
