...
- **Advertised path strings.** The literal un-expanded constants `HONEYCOMB_REGISTRY_PID_PATH = "~/.honeycomb/daemon.pid"` (`fleet-registry.ts:60`) and `HONEYCOMB_REGISTRY_TELEMETRY_DB_PATH = "~/.honeycomb/telemetry/honeycomb.sqlite"` (`fleet-registry.ts:66`) flip to the `~/.apiary/...` equivalents in the SAME release that 072a moves the pid and 072b moves the SQLite, preserving the documented invariant that the advertised string and `fleetTelemetryDbPath` name the same on-disk file (`fleet-registry.ts:18-24`). RESOLVED per fleet ADR-0003 Resolved decision 4 (2026-07-04): the registry contract carries RESOLVED ABSOLUTE paths derived from the writer's own `resolveFleetRoot`; doctor expands legacy tilde-literals during the window only. Known accepted corner (QA re-audit note, 2026-07-04): in the telemetry mover-failure fallback state the registry advertises the new `telemetryDbPath` while writes temporarily land at the legacy DB; this self-heals on the next successful move, and doctor tolerates an advertised path with no file (the probe degrades, never throws).
