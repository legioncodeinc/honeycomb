...
- **Bring up the daemon** on its port (3850) via the existing ensure-running path ([`src/commands/daemon.ts`](../../../../src/daemon/runtime/dashboard/host.ts) `ensureDaemonRunning`), wait on `/health`, and **open the dashboard** in the default browser — at `honeycomb.local` when resolvable, else the `http://127.0.0.1:3850/dashboard` loopback fallback.
