...
| d-AC-7 | Given a daemon crash/kill **mid-migration**, on restart `GET /setup/state` reports the non-terminal `migration.phase` (not a clean state) and the dashboard offers **resume** or **roll back** (restore the backup) — a half-migrated machine is never presented as cleanly migrated or cleanly reverted. |
...
- **Migration is one guarded transaction in the daemon, with a persisted recovery marker** — a backup+uninstall flow is only "reversible" if a crash *between* steps is recoverable. Before any destructive step, write a durable `migration: { phase, startedAt, backupPath }` marker (in the onboarding/setup-state file, fail-soft like the rest of it) and advance its `phase` through each step: `backing_up → uninstalling → relinking → done` (or `rolled_back`). The transaction is: write marker `backing_up` → back up → `uninstalling` → uninstall → re-check credential → `relinking` → (adopt | login) → `done` + stamp `hivemind: migrated`. On daemon restart, `GET /setup/state` reads the marker: a non-terminal phase means "migration interrupted at X" — the dashboard offers **resume** or **roll back** (restore the backup) rather than silently presenting a half-migrated machine as clean. Each step logs a plain line; any failure stops with a recoverable message + the marker phase + the backup path, never a trace.
