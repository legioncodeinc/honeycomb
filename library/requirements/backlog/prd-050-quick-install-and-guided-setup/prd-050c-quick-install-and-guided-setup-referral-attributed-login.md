...
- **Default `ref = mario`** for Honeycomb installs: the `install` / "First time setup" / "Link to DeepLake" paths bind `mario` unless an explicit `--ref <code>` overrides it — attribution is automatic, never something the user must supply.
