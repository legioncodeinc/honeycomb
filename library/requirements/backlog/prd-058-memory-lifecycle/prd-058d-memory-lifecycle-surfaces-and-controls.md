# PRD-058d: Lifecycle config, audit, dashboard, and CLI surfaces
...
Give the four lifecycle engines (recency/activation, conflicts, stale-references, calibration) one coherent operator surface so the whole model in [`memory-lifecycle-scoring.md`](../../../knowledge/private/ai/memory-lifecycle-scoring.md) is configurable, visible, and auditable from one place. Where 058a/058b/058c/058e *produce* the terms of the master equation `P(m | q, t) = R · A^a · C^c · (1 − σ)^s · κ`, 058d is the **operator surface for all of them**: a single typed `memory.lifecycle.*` config block (with `HONEYCOMB_LIFECYCLE_*` env overrides) that governs every exponent and parameter, an audit trail through `memory_history`, a lifecycle dashboard panel on the memories page that renders the query-independent health scalar `H(m,t)` plus per-term health (freshness, open-conflict count, stale-ref count, calibration ECE), and CLI parity for inspect-and-resolve. Each engine ships behind an explicit flag with a non-destructive default, so an operator opts in deliberately and can always see what a lifecycle action did and why, and can reverse it.

This sub-PRD owns no detection or resolution math. It owns the *knobs*, the *windows*, and the *audit*. The design rule it inherits from the scoring doc is the one that makes the whole model safe to ship: every term is a bounded multiplier that can only demote, and every unproven term ships dormant (exponent `0`, the identity). 058d's job is to make that posture the install default and to make any change away from it deliberate, documented, and visible.

### What it surfaces

058d configures and renders every term of the master equation. The table below reproduces the scoring doc's "Parameters and defaults" verbatim and maps each symbol to its `memory.lifecycle.*` config path and its `HONEYCOMB_LIFECYCLE_*` env override. The owning sub-PRD computes the value; 058d exposes the knob, the default, and the effect on the settings page and in the config reference.

| Parameter | Symbol | Master-equation role | Default | Config path (`memory.lifecycle.*`) | Env override (`HONEYCOMB_LIFECYCLE_*`) | Owner |
|---|---|---|---|---|---|---|
| Activation exponent | `a` | `A^a` | `1.0` | `activationExponent` | `ACTIVATION_EXPONENT` | 058a |
| Confidence exponent | `c` | `C^c` | `0` (dormant until calibrated) | `confidenceExponent` | `CONFIDENCE_EXPONENT` | 058e |
| Staleness exponent | `s` | `(1 − σ)^s` | `0` under `observe`, `1` under `execute` | `stalenessExponent` | `STALENESS_EXPONENT` | 058c |
| Half-life, distilled | `h(memories)` | shapes `A` | `180 d` | `halfLifeDaysByClass.memories` | `HALFLIFE_MEMORIES_DAYS` | 058a |
| Half-life, summary | `h(memory)` | shapes `A` | `45 d` | `halfLifeDaysByClass.memory` | `HALFLIFE_MEMORY_DAYS` | 058a |
| Half-life, raw | `h(sessions)` | shapes `A` | `10 d` | `halfLifeDaysByClass.sessions` | `HALFLIFE_SESSIONS_DAYS` | 058a |
| ACT-R decay | `d` | shapes `A` (Stage 2) | `0.5` | `actrDecay` | `ACTR_DECAY` | 058e |
| Activation floor | `A_min` | clamps `A` | `0.05` | `activationFloor` | `ACTIVATION_FLOOR` | 058e |
| Verification half-life | `h_verify` | shapes `σ` via `v(m,t)` | `14 d` | `verificationHalfLifeDays` | `VERIFICATION_HALFLIFE_DAYS` | 058c |
| Contradiction threshold | `θ_detect` | gates conflict detection | `0.6` | `contradictionThreshold` | `CONTRADICTION_THRESHOLD` | 058b |
| Corroboration weight | `γ` | shapes conflict vote `w_i` | `0.5` | `corroborationWeight` | `CORROBORATION_WEIGHT` | 058b |
| Supersede margin | `τ_supersede` | conflict verdict cut | `0.5` | `supersedeMargin` | `SUPERSEDE_MARGIN` | 058b |
| Review margin | `τ_review` | conflict verdict cut | `0.15` | `reviewMargin` | `REVIEW_MARGIN` | 058b |
| Open-conflict suppression | `ρ` | `κ` for the open-conflict loser | `0` (fully suppress, reversible) | `openConflictSuppression` | `OPEN_CONFLICT_SUPPRESSION` | 058b |
...
`H` is computed read-side from `freshnessScore`/`activation` (`A`), `calibratedConfidence` (`C`), the stale-ref status (`σ`), and the conflict gate (`κ`). 058d adds no new write path to produce it; it is a pure projection of fields the other terms emit.
...
- **Dashboard.** A lifecycle panel on the memories page: a per-memory health badge rendering `H`, a freshness indicator (from 058a/058e `A`), an open-conflict count and a per-conflict resolve action (from 058b), a stale-reference list and count (from 058c), and a calibration view (ECE plus a reliability diagram, from 058e).
...
- The detection and resolution logic itself. Activation and decay belong to 058a, the conflict detector/resolver and the `κ` gate belong to 058b, the stale-reference resolver and `σ` belong to 058c, and reinforcement, ACT-R activation, and the calibration fit belong to 058e. 058d surfaces their outputs and configures their knobs; it computes none of them.
- New auth roles. Operator-scope resolution reuses the existing RBAC from PRD-011.
- Any new write path on the capture or controlled-write hot path. The only write 058d invokes is the already-defined 058b resolve endpoint.

### Dependencies

- **Blocked by:** sub-PRDs 058a (`freshnessScore`, the `a` exponent), 058b (`memory_conflicts`, the resolve endpoint, `κ`, the conflict parameters), 058c (`ref_status`/`stale_refs`/`verified_at`, `σ`, `h_verify`), 058e (`activation`, `calibratedConfidence`, `memory_calibration`, the reliability-diagram payload). This sub-PRD surfaces their outputs and cannot ship a term's control before that term emits a value.
...
- AC-55d.2.2 Given a conflict in the queue, when I resolve it from the dashboard, then it calls the 058b `POST .../resolve` endpoint and the row updates without a reload race, polling to convergence per the eventual-consistency rule, never a single immediate read-back.
...
- AC-55d.2.4 Given the calibration curve has been fit, when I open the calibration view, then it renders the reliability diagram and the current ECE/Brier from `memory_calibration` (the 058e introspection payload), so an operator can see whether stated confidence is trustworthy before relying on `c`.
...
- AC-55d.3.2 Given a conflict id, when I run `honeycomb memory conflicts resolve <id> --verdict supersede --winner <id>`, then it resolves through the same 058b endpoint and code path as the dashboard, with no parallel resolve logic.
...
None. This sub-PRD reads, it does not define. It consumes `memory_conflicts` (058b), the `ref_status` / `verified_at` / `stale_refs` columns (058c), `freshnessScore` / `activation` / `calibratedConfidence` (058a/058e, response-only fields), `memory_history` (existing, the audit substrate), and `memory_calibration` (058e, the reliability-diagram source). The health scalar `H(m,t)` is computed read-side from these fields and is not persisted.
...
- **Reuses** `POST /api/memories/conflicts/:id/resolve` (defined in 058b) for every resolve, from both dashboard and CLI. 058d defines no new write path.
...
- `GET /api/memories/calibration` - the current calibration curve's `ece`, `brier`, `n_samples`, and reliability-diagram payload (defined in 058e), surfaced for the calibration view (scoped).

The per-memory health fields (`freshnessScore`, `activation`, `calibratedConfidence`, ref status, conflict gate) ride the existing recall and memory-detail responses defined by 058a/058b/058c/058e; `H` is assembled from them on the read side.
...
- **No new write path on the hot path.** Every surface here is a read plus the one already-defined 058b resolve endpoint. Nothing in 058d touches the capture or controlled-write path, so the rule that a slow or failing step must never cost the user a memory is preserved.
- **`H` is a read-side projection.** The health scalar is computed from the already-emitted `A`/`C`/`σ`/`κ` fields at render time. 058d adds no column, no job, and no aggregation step to produce it; if a term is dormant its factor is the identity, so `H` degrades gracefully to the terms that are live.
...
- **Integration:** dashboard conflict resolve round-trips through the 058b `POST .../resolve` endpoint and polls to convergence; CLI resolve hits the same path (AC-55d.3.2); scope enforcement rejects out-of-scope conflict/stale-ref/history/calibration reads before any content is returned; the calibration view renders the 058e reliability-diagram payload.
...
- [`prd-058-memory-lifecycle-index.md`](./prd-058-memory-lifecycle-index.md) - the parent PRD index.
