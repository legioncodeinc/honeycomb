# PRD-063h: HiveDoctor - Primary Daemon as an OS-Native Service
...
- Per-OS service templates for the primary daemon (sibling to 063b's HiveDoctor templates; same approach, separate units), pinning the writable workspace + `HONEYCOMB_WORKSPACE` so the daemon never boots from `C:\WINDOWS\system32` (the documented "secrets 502" failure).
...
- HiveDoctor's own service - [063b](./prd-063b-hivedoctor-self-healing-watchdog-self-supervision-and-install-integration.md).
- The healing logic that sits above the service - [063a](./prd-063a-hivedoctor-self-healing-watchdog-supervisor-core-and-lifecycle.md)/[063c](./prd-063c-hivedoctor-self-healing-watchdog-remediation-ladder.md).
- The embeddings child, which the primary daemon keeps supervising itself (OD-8 indirect).

## Acceptance criteria

- AC-063h.1 Given a clean install, when bootstrap completes, then the primary daemon runs as an OS service and answers `/health`.
- AC-063h.2 Given the daemon process is killed, when the OS service manager notices, then it restarts the daemon without HiveDoctor having to intervene (liveness floor).
- AC-063h.3 Given a reboot, when the machine returns, then the daemon starts automatically.
- AC-063h.4 Given the service starts the daemon, when it does, then cwd/`HONEYCOMB_WORKSPACE` is a writable repo-root workspace (never `system32`), closing the "secrets 502" class.
- AC-063h.5 Given HiveDoctor performs a rung-1 restart, when it does, then it goes through the service manager and the PID/lock guard prevents any double-bind.
- AC-063h.6 Given `honeycomb` start/stop/status, when run, then they reflect and control the service state (not a stray detached process).
...
- **Don't double-supervise the same way:** with the daemon OS-managed, HiveDoctor must restart *via* the service, not by spawning - otherwise the two race. This is the main reason to do 063h alongside HiveDoctor rather than later.
...
- [ ] Sequencing: ship 063h with HiveDoctor v1, or land the daemon-service first as a standalone hardening step?
