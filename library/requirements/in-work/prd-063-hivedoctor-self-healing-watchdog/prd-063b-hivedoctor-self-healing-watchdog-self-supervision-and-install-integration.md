# PRD-063b: HiveDoctor - Self-Supervision and Install Integration
...
- This sub-PRD registers HiveDoctor's own OS service. The Honeycomb **primary daemon** also becomes OS-native in the sibling sub-PRD [063h](./prd-063h-hivedoctor-self-healing-watchdog-primary-daemon-os-native-service.md); the two share the per-OS service-template approach but are independent units.
...
- The watch loop itself - [063a](./prd-063a-hivedoctor-self-healing-watchdog-supervisor-core-and-lifecycle.md).
- What telemetry is emitted - [063d](./prd-063d-hivedoctor-self-healing-watchdog-telemetry-and-observability.md).

## Acceptance criteria

- AC-063b.1 Given a clean install on each OS, when bootstrap completes (without opt-out), then a HiveDoctor service is registered and running.
- AC-063b.2 Given HiveDoctor is SIGKILLed, when the service manager notices, then it restarts HiveDoctor within the configured window (AC-1 parent).
- AC-063b.3 Given a reboot, when the machine comes back, then HiveDoctor starts automatically before/independently of the primary daemon.
- AC-063b.4 Given `--no-hivedoctor` at install, when bootstrap completes, then no service is registered and no HiveDoctor process runs (AC-10 parent).
- AC-063b.5 Given `hivedoctor uninstall-service`, when run, then the OS unit is removed and does not resurrect on next boot.
- AC-063b.6 Given a non-admin/unprivileged context, when service registration is not possible, then HiveDoctor falls back to a userland-scoped service (systemd `--user`, launchd LaunchAgent, Windows per-user Scheduled Task) rather than failing the install.
...
> OD-1 (OS-native), OD-5 (master switch + dashboard toggles), and the Windows default (**per-user Scheduled Task**; Windows Service as enterprise opt-in, shared with [063h](./prd-063h-hivedoctor-self-healing-watchdog-primary-daemon-os-native-service.md)) are resolved in the parent index.
