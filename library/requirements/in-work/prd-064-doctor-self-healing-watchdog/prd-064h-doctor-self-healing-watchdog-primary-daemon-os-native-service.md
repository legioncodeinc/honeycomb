# PRD-064h: Doctor - Primary Daemon as an OS-Native Service
...
Per OD-1 (Mario, 2026-06-27): the Honeycomb **primary daemon** should itself be OS-native, not just spawned-and-detached by the CLI. The OS service manager becomes the **liveness floor** - it restarts the daemon on crash and starts it on boot - while Doctor remains the **intelligent healing layer** above it (handles wedged-but-alive, stale routes, bad installs, version updates, and escalation, which a dumb OS restart cannot).
...
- Make Doctor's rung-1 restart go *through* the service manager (`launchctl kickstart` / `systemctl --user restart` / `Restart-Service` / task re-run) rather than spawning a second process - eliminating the watchdog-war / double-bind risk.

## Scope

- Per-OS service templates for the primary daemon (sibling to 064b's Doctor templates; same approach, separate units), pinning the writable workspace + `HONEYCOMB_WORKSPACE` so the daemon never boots from `C:\WINDOWS\system32` (the documented "secrets 502" failure).
...
- Doctor's own service - [064b](./prd-064b-doctor-self-healing-watchdog-self-supervision-and-install-integration.md).
- The healing logic that sits above the service - [064a](./prd-064a-doctor-self-healing-watchdog-supervisor-core-and-lifecycle.md)/[064c](./prd-064c-doctor-self-healing-watchdog-remediation-ladder.md).
...
- AC-064h.2 Given the daemon process is killed, when the OS service manager notices, then it restarts the daemon without Doctor having to intervene (liveness floor).
...
- AC-064h.5 Given Doctor performs a rung-1 restart, when it does, then it goes through the service manager and the PID/lock guard prevents any double-bind.
...
- **Division of labor:** OS service = "process is running"; Doctor = "the install is actually healthy and current." Both are needed - a daemon can be running and still wedged (the memory_jobs backlog wedge), which the OS cannot detect but Doctor's `/health` probe can.
- **Don't double-supervise the same way:** with the daemon OS-managed, Doctor must restart *via* the service, not by spawning - otherwise the two race. This is the main reason to do 064h alongside Doctor rather than later.
...
- [ ] Sequencing: ship 064h with Doctor v1, or land the daemon-service first as a standalone hardening step?
