# PRD-064d: Doctor - Telemetry and Observability
...
1. **Errors (severity ERROR)** - when Doctor catches an error or a remediation fails. Optionally also pushed to PostHog Error Tracking (`captureException` → `$exception`) for issue-grouping.
...
- Each log record carries `service.name` (`doctor`), `device_id` (PRD-033 UUID, for broken-auth correlation), daemon + Doctor versions, and the OS - via OTLP resource + record attributes.
...
- The dashboard rendering of escalations - [064g](./prd-064g-doctor-self-healing-watchdog-dashboard-escalation-reporting.md).
- Generating the incidents themselves - [064a](./prd-064a-doctor-self-healing-watchdog-supervisor-core-and-lifecycle.md)/[064c](./prd-064c-doctor-self-healing-watchdog-remediation-ladder.md).

## Acceptance criteria

- AC-064d.1 Given default settings, when Doctor catches an error, then a scrubbed ERROR-severity OTLP log record reaches PostHog Logs at `/i/v1/logs`.
...
- AC-064d.6 Given the telemetry sink is unreachable, when emission fails, then Doctor swallows the error and continues healing (telemetry never blocks).

## Technical considerations

- **PostHog Logs is confirmed (OD-2 + transport resolved):** a generic OTLP receiver at `/i/v1/logs`, `phc_` project-token auth, OTLP/HTTP+JSON. No collector, no Sentry, no second vendor. One project holds Doctor's logs alongside the product's events and Error Tracking.
- **Hand-rolled, zero-dep:** because the logs path accepts OTLP/JSON, Doctor serializes the `LogsData` JSON by hand and POSTs via `fetch`. This keeps the can't-crash process free of the OpenTelemetry SDK. Vendor a tiny, well-tested serializer module; validate its JSON against the OTLP logs schema in a Vitest fixture.
...
- **Alpha caveat + cost:** PostHog Logs is alpha (pin the endpoint behind one constant); free to 50GB/mo, far above Doctor's low-volume episodic output.
