...
|---|---|---|
| 064a.1 healthy -> no action, low-verbosity | V | `supervisor.test.ts` (ok branch). |
| 064a.2 unreachable -> rung1 restart, next probe healthy, backoff resets | V | `supervisor.test.ts` + `backoff.test.ts`. |
| 064a.3 3 failed restarts -> advance to rung 2 | V | `supervisor-escalation.test.ts` (threshold=3 advance). |
| 064a.4 subsystem reason -> targeted rung | V | `health-probe.test.ts` classification + `supervisor.test.ts`. |
| 064a.5 step throws -> caught, recorded, loop continues | V | `supervisor.test.ts`. |
| 064a.6 cooldown respects daemon pid/lock | V | `rungs.test.ts` (restart rung cooldown/skip). |

### 064b Self-supervision + install
...
### 064c Remediation ladder

| AC | Status | Proving test |
|---|---|---|
| 064c.1 3 fails -> reinstall, stale-route gone (version matches blessed) | V (logic) - see W-1 | `remediation-064c.test.ts` (rung injects real blessed). Production compose verify is no-op (W-1). |
| 064c.2 conflicting hivemind removed, `~/.deeplake/` intact | V | `rungs.test.ts` (package-only uninstall; no deeplake write). |
| 064c.3 suspected cred fault -> escalate, no delete | V | no purge path exists; `remediation-064c.test.ts`. |
| 064c.4 rung re-run = safe no-op (idempotent) | V | `rungs.test.ts` (detect-first skip). |
| 064c.5 rung 3 records what was removed before deletion | V | `rungs.test.ts` (backup-before, refuses if backup fails). |
| 064c.6 before/after state in incidents.ndjson | V | `incidents.test.ts`, `supervisor-escalation.test.ts`. |

### 064d Telemetry

| AC | Status | Proving test |
|---|---|---|
| 064d.1 error -> scrubbed ERROR OTLP at `/i/v1/logs` | V (shape) / B (B-4 live ingest) | `telemetry/emit.test.ts`, `otlp-serializer.test.ts`. |
| 064d.2 install-health INFO record | V (shape) / B (B-4) | `emit.test.ts`. |
| 064d.3 episode record w/ steps + device_id | V (shape) / B (B-4) | `emit.test.ts`. |
| 064d.4 opt-out -> nothing leaves the box | V | `emit.test.ts` (3 gates). |
| 064d.5 no credential/token/PII (allow-list) | V | `emit.test.ts` (closed allow-list + banned-key absence). |
| 064d.6 sink unreachable -> swallow, keep healing | V | `emit.test.ts` (send_failed path). |
| 064d.7 no OTel SDK dep (hand-rolled) | V | zero deps in package.json; `otlp-serializer.test.ts`. |

### 064e Auto-update

| AC | Status | Proving test |
|---|---|---|
| 064e.1 blessed newer -> update within ~30m | V (logic) / B (B-2) | `update-engine.test.ts`, `poll-loop.test.ts`. |
| 064e.2 latest newer but NOT blessed -> no update | V | `update-policy.test.ts` (`latest_not_blessed`). |
| 064e.3 post-update health fails -> rollback | V | `update-engine.test.ts` (rollback to prior, healthy again). |
| 064e.4 `--no-auto-update` / pin -> no update | V | `update-policy.test.ts`, `opt-out.test.ts`. |
| 064e.5 update/rollback emits from/to/outcome | V | `update-engine.test.ts`, `update-telemetry.test.ts`. |
| 064e.6 update + watch loop serialized (install lock) | V | `install-lock.test.ts`, `update-engine.test.ts` (skipped_lock_held). |

### 064f CLI + UX

| AC | Status | Proving test |
|---|---|---|
| 064f.1 no-args -> ASCII art + menu | V | `banner.test.ts`, `command-table.test.ts`. |
| 064f.2 `status` -> health/service/versions/last-heal/opt-out | V | `dispatch.test.ts`. |
| 064f.3 `diagnose` -> recommend rung, NO action | V | `dispatch.test.ts` (no-action asserted). |
| 064f.4 `uninstall-hivemind` confirms, never deletes deeplake | V | `dispatch.test.ts`, `rungs.test.ts`. |
| 064f.5 `self-update` is the ONLY self-install path | V | grep + `self-update.test.ts`. |
| 064f.6 `status`/`diagnose` work when daemon down | V | `dispatch.test.ts` (daemon-down still reports). |

### 064g Escalation reporting

| AC | Status | Proving test |
|---|---|---|
| 064g.1 ladder exhausts -> structured needs-attention persisted | V | `needs-attention-store.test.ts`, `supervisor-escalation.test.ts`. |
| 064g.2 daemon recovers -> dashboard renders report | V (file+seam) / B (live dashboard render) | store + incidents seam; daemon-side render is a later dashboard task (plan-acknowledged). |
| 064g.3 credentialed + sink on -> reaches hosted surface | V (logic) / B (B-4) | `hosted-sink.test.ts`. |
| 064g.4 daemon down + status page on -> health + escalation + commands | V | `status-page/server.test.ts`. |
| 064g.5 resolution marks report resolved | V | `needs-attention-store.test.ts` (resolve()). |

### 064h Daemon OS-native

| AC | Status | Proving test |
|---|---|---|
| 064h.1 clean install -> daemon as OS service answers /health | B (B-1) | service-aware lifecycle unit-tested; live blocked. |
| 064h.2 killed -> OS manager restarts (liveness floor) | B (B-1) | templates assert restart policy; live blocked. |
| 064h.3 reboot -> auto-start | B (B-1) | unit; live blocked. |
| 064h.4 service start -> writable workspace, never system32 | V (unit) | `daemon-service.test.ts` + `resolveDaemonWorkspace` write-probe; the documented 502 fix. |
| 064h.5 rung-1 restart goes through manager, no double-bind | V (unit) | `daemon-lifecycle-service.test.ts` (restart via manager). |
| 064h.6 `honeycomb` start/stop/status reflect/control service | V (unit) | `daemon-lifecycle-service.test.ts` (service-preferred + spawn fallback + fail-open). |
...
- **The one inflation-risk claim is the 064h "3963 pass/1 known flake" note** - which is *correct*: the 1 failure is a pre-existing flaky timeout in an unrelated property test (verified by isolation), exactly as the ledger characterizes it.
...
- `src/remediation.ts` - ladder + rung registry + escalation record builder (064c).
- `src/rungs/{reinstall,uninstall-hivemind,escalation,command-runner}.ts` - rungs 2/3/4 + fixed-argv runner (064c).
- `src/install-lock.ts` - shared exclusive-create mutex (064c/064e).
- `src/update/{update-policy,update-engine,blessed-channel,poll-loop,registry,version,update-telemetry}.ts` - fail-closed blessed-gated auto-update + verify/rollback (064e).
...
- `src/cli/daemon-service.ts` (NEW) - primary daemon OS-service (launchd/systemd/schtasks), writable-workspace pinned, `assertCmdSafe` (064h + security High-2 fix).
- `src/cli/runtime.ts` - `buildDaemonLifecycle` service-aware + restart-through-manager + spawn fallback (064h).
- `src/commands/install.ts` - additive supervision report (064h).
