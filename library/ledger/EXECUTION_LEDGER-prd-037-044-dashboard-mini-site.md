...
- **Post-push review + conflict resolution (PR #70):**
  ...
  - **3 CodeRabbit fixes** (bcf35e6): dream() synchronous useRef re-entry guard; identity re-hydrate only on down→up recovery; test `afterEach` unmount guard. Replied + resolved all 3 threads.
