# Execution Ledger - PRD-064 Doctor

> /the-smoker run ledger for PRD-064 (Doctor self-healing watchdog).
...
2. **Live external services** - npm publish of `@legioncodeinc/doctor`, a real `blessed-version.json` on `get.theapiary.sh`, live PostHog Logs ingestion (alpha + `phc_` token).
...
- **Wave 0 (serial, foundation):** scaffold `doctor/` package (dependency-light, strict ESM Node 22, own tsconfig, Vitest), shared state/incident model + logger. Owner: `typescript-node-worker-bee`. Unblocks all.
...
| M-10 | install opt-out (`--no-doctor`) | OPEN | harness-integration |
...
> doctor/ package scaffolded (zero runtime deps, strict ESM Node22, own tsconfig+vitest). 49 tests pass; root typecheck/dup/test unaffected. Files: doctor/{package.json,tsconfig.json,vitest.config.ts,README.md}, src/{config,state,incidents,logger,health-probe,backoff,remediation,supervisor}.ts, tests/*. Rungs 2+ are declared interface slots for later waves. M-2 and M-8 now substantially covered.
...
> doctor/src/service/{platform,templates,argv,index,install-guard}.ts (launchd/systemd/schtasks, userland default + privileged fallback, all shell-outs injected). CLI install-service/uninstall-service now live + `run` long-running entry. Installer hooks added to install.sh+install.ps1 (additive, --no-doctor guarded); src/commands/install.ts untouched. 378 tests. M-10 DONE.
...
> hand-rolled OTLP Logs JSON to /i/v1/logs, single chokepoint, opt-out (DO_NOT_TRACK/HONEYCOMB_TELEMETRY/state toggle), allow-list scrub, fail-soft. Files: doctor/src/telemetry/{globals.d.ts,otlp-serializer.ts,emit.ts} + tests. 121 total tests green. Live ingestion needs phc_ token at build/publish (B-4).
...
> 30-min jittered poll, blessed-version.json gate (fail-closed), verify /health + rollback, shared install-lock serialization, injectable UpdateEmit seam onto 064d. Files: doctor/src/update/* + tests. 237 tests. WIRING TODO (064f composition root): resolve --no-auto-update/env/state pin precedence + start poll loop.
...
INT-1 DONE - doctor esbuild build (bundle/cli.js), files allowlist (5-file tarball), prepack, pack-check; multi-OS CI matrix (ubuntu/macos/windows) added to ci.yaml (additive); root tsc/dup/vitest isolated + untouched.
INT-2 DONE(authored)/BLOCKED(live) - .github/workflows/release-doctor.yaml (OIDC trusted publishing, provenance, doctor-v* tag, fail-closed, does NOT publish). First publish is manual bootstrap (B-2) + version bump off 0.0.0.

## Close-out
- security-worker-bee: DONE - 2 High remediated in place (rollback semver validation in update-engine.ts; Windows schtasks /TR command-injection in daemon-service.ts via assertCmdSafe), 0 Critical. Tests green (378 doctor + 27 daemon-service). Report: qa/prd-064-security-report.md. Medium/Low follow-ups noted.
...
- remediation pass (W-1/W-2/S-2): DONE - blessed version threaded into rung-2 verify (fail-soft/fail-closed, degrades to unverified-no-blessed until B-3); systemd ExecStart quoted; all PRD-064-authored em/en dashes removed. 384 doctor tests + 43 daemon tests green; root typecheck/dup pass; dash grep clean. Close-out CLEAN (0 Critical, 0 open Warnings >= medium).
- SHIP: committed + pushed; PR #159 open (https://github.com/legioncodeinc/honeycomb/pull/159).
  - B-6 RESOLVED: PRD number collision (origin/main shipped prd-063 = Portkey Gateway). Renumbered this Doctor PRD 063 -> 064; merged origin/main clean; pushed. The pull_request CI workflow now schedules. Doctor gate PASSES on macos+ubuntu+windows; CodeQL/Secret/CLA green; Quality gate + Windows smoke running.
  - B-7 AIKIDO: re-audited (qa/prd-064-aikido-triage.md). 0 of 4 CRITICAL and 0 of 8 HIGH exploitable. The 2 genuine TRUE-POSITIVES were already fixed in the security pass (semver-gated rollback npm spec; assertCmdSafe on schtasks /TR). Rest are SAST shape-matches (by-design / false-positive) with paste-ready suppression rationales. ACTION NEEDED (Mario): suppress the false-positives in the Aikido dashboard for that gate to go green; cannot be done from here.
...
- **B-4 PostHog token:** confirm `phc_` project token injection for Doctor + that PostHog Logs alpha is acceptable.
...
- Wave 0 DONE: doctor/ package + 064a supervisor core, 49 tests green, root gates intact, fully additive (no src/ changes). Checkpoint: holding before Waves 1-3 pending Mario's rulings on B-1..B-5 (publish, 064h ship gate, OS matrix, PostHog token, ship target).
