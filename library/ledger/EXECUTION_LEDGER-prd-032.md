...
> `setting` (active provider/model, dreaming toggle, dashboard prefs), + an extensible class seam — surfaced
> via CLI + dashboard and wired back so provider/model + dreaming are vault-driven. EXTEND the PRD-012 crypto
...
- Wire-back targets: `inference/config.ts` `resolveInferenceConfig` + `model-client-factory.ts` `buildInferenceModelClient` (reads the committed `agent.yaml`), `dreaming/config.ts` (`HONEYCOMB_DREAMING_ENABLED`), `assemble.ts` (`buildGatedDreamingWorker`, `AGENT_CONFIG_FILE_NAME`, `buildInferenceModelClient` call). DeepLake login: `auth/credentials-store.ts` (`~/.deeplake/credentials.json`, the SHARED Hivemind login — migration is COPY-not-move).
...
| AC-5 | Dashboard Settings panel: provider (Anthropic/OpenAI/OpenRouter)→catalog model list→pick; dreaming on/off toggle; writes via daemon endpoint to the `setting` class; reload reflects persisted value; NO secret value rendered. | VERIFIED | 032c |
| AC-6 | Wire-back: provider/model + dreaming vault-DRIVEN (vault wins over `agent.yaml`/env); with vault `dreaming.enabled` true, `/api/diagnostics/dream` not `disabled` WITHOUT the env var; no vault setting → falls back (no regression). | VERIFIED | 032d |
...
- 032c dashboard (`typescript-node-worker-bee`): Settings panel (provider→model + dreaming toggle) → `/api/settings`; no secret rendered. AC-5. Owns `src/dashboard/web`.
- 032d wire-back (`typescript-node-worker-bee`): assembly reads provider/model + dreaming-enabled from the vault (vault wins; `agent.yaml`/env fallback). AC-6. Owns `assemble.ts`.
...
- Wave 2 DONE: 032b `settings` CLI verb + provider/model selector (AC-4, 21 tests); 032c dashboard Settings panel (AC-5, 18 DOM/wire tests, no-secret-render proof); 032d assembly vault-driven provider/model + dreaming wire-back (AC-6).
...
- Wave 2 / 032d wire-back (AC-6 / D-5) DONE. `src/daemon/runtime/assemble.ts` now READS the vault `setting` class
  ...
  - **Provider/model vault-driven (FR-1):** `buildGatedDreamingWorker` reads `activeProvider` + `activeModel` from the
  ...
  - **Dreaming vault-driven (FR-3 / d-AC-2):** precedence is VAULT-FIRST — when the vault `setting` `dreaming.enabled`
    is present it WINS (a vault `true` enables dreaming WITHOUT `HONEYCOMB_DREAMING_ENABLED`; a vault `false` disables
    even when the env says true); absent it, the gate falls back to `resolveDreamingConfig().enabled` (PRD-026 env).
  ...
  - **Tests:** `tests/daemon/runtime/inference/model-client-factory.test.ts` (+2: d-AC-1 override-wins, d-AC-3 no-override
    no-regression) and `tests/daemon/runtime/assemble.test.ts` (+7: vault-first dreaming true/false precedence, env
