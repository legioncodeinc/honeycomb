...
| c-AC-1 | 059c | Projects page lists projects + state (paths, remote, counts) | dashboard UI | react-worker-bee | DONE (UI) — `ProjectsPage` lists locally-bound projects (name + state cells). NOTE: paths/remote/last-capture/counts are NOT served by the Wave-1 `scope/projects` read (only `projectId`/`name`/`boundLocally`), so those cells render honest "—" placeholders; the per-project aggregate daemon read is a deferred follow-up (`pages/projects.tsx`; `projects-page.test.tsx`) |
| c-AC-2 | 059c | `__unsorted__` inbox shown distinctly with size | dashboard UI | react-worker-bee | DONE — the inbox renders as its own distinct dashed row separate from active projects; size shown as "—" (row count not served by the Wave-1 read) (`pages/projects.tsx`; `projects-page.test.tsx`) |
