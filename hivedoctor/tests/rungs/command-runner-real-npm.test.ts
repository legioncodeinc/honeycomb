 ...
 * no network, no global state. A generous timeout absorbs a cold npm start on a busy CI runner.
 */
...
describe("createExecFileRunner against real npm (multi-OS smoke)", () => {
	it("runs `npm --version` and returns ok with a parseable semver", async () => {
		const runner = createExecFileRunner();
		const result = await runner.run("npm", ["--version"], { timeoutMs: 60_000 });
	...
	it("runs `npm ls -g <pkg> --depth=0 --json` and returns parseable JSON (the installed-version path)", async () => {
