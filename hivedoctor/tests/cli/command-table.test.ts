/**
 * Command-table tests (PRD-063f Scope; AC-063f.4 deferred-action enforcement).
 *
...
describe("command table (PRD-063f)", () => {
	it("includes self-update (the sole self-update path, AC-063f.5)", () => {
		expect(KNOWN_COMMANDS.has("self-update")).toBe(true);
	});

	it("does NOT include clear-credentials (deferred, OD-4 / AC-063f.4)", () => {
	...
	it("lists every PRD-063f command", () => {
		for (const cmd of [
			"status",
			"diagnose",
			"heal",
