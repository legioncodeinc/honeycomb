/**
 * The auto-update gate decision (PRD-063e AC-063e.1 / .2 / .4).
 *
 ...
 *
 * The gate (in order):
 *   1. Opt-out / pin   -> no update (AC-063e.4: `--no-auto-update`, env, or a pin).
 ...
 *   4. latest is NOT blessed (latest !== blessed)        -> no update (AC-063e.2).
 ...
 * install is pinned to the audited bits (PRD-063e: "npm i -g <blessed>").
 */
...
export interface UpdateOptOut {
	 ...
	 * gate only needs the boolean (AC-063e.4).
	 */
	readonly autoUpdateDisabled: boolean;
	/**
	 * An optional pinned version. When set, forward updates are disabled: the daemon stays
	 * on the pin regardless of what is blessed (AC-063e.4, "a pinned version that disables
...
export interface UpdateDecisionInput {
	...
	/** Opt-out + pin inputs (AC-063e.4). */
	readonly optOut: UpdateOptOut;
}

/** Why the gate declined to update (each maps to an AC). */
export type NoUpdateReason =
	| "opted_out" // AC-063e.4: auto-update disabled
	| "pinned" // AC-063e.4: a pin disables forward updates
	| "blessed_unavailable" // AC-063e.2 (fail-closed half): channel unreachable/unparseable
	| "latest_unknown" // registry read failed this tick
	| "latest_not_blessed" // AC-063e.2: @latest is newer but not blessed
...
export function decideUpdate(input: UpdateDecisionInput): UpdateDecision {
	// 1. Opt-out / pin first -- these short-circuit before any version reasoning (AC-063e.4).
	...
	// 2. Fail-closed: a blessed channel we could not read means stay on current (AC-063e.2).
	...
	//    (AC-063e.2): a fresh publish that has not been blessed for rollout is ignored.
