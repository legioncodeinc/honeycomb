...
- `tokens/` — colors (honey/dream/verified + surfaces), typography (Inter + JetBrains Mono), spacing/radii/motion, `@font-face`.
...
## Non-negotiables
- Dark-native, warm near-black canvas. Honey amber `#F7A823` is the one brand hue — **one saturated honey region per view** (scarcity rule). Dream violet is only for the Dreaming loop; verified-green only for source-backed states.
...
- Cards = 1px border on `bg.elevated`, 12px radius, no shadow. The only expressive light is the honey/dream glow on one focused element.
